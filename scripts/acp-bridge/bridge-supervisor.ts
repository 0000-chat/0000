...
export type BridgeSupervisorHealth =
  | { status: "healthy" }
  | { status: "hard_failed"; reasonCode: BridgeJournalReasonCode; message: string }
...
export function openBridgeSupervisor(options: OpenBridgeSupervisorOptions): BridgeSupervisor {
  try {
...
export class BridgeSupervisor {
  ...
  recordProviderEvent(item: BridgeSupervisorWorkItem, event: { eventType?: string } = {}): void {
    this.retirePromptOutbox(item, { providerEventType: event.eventType })
    this.transition(item, "active", { lastProviderEventAt: this.now() })
    this.appendLocalDiagnostic(item, "provider_event", event.eventType ?? "provider event observed")
  }

  recordWaitingForInteraction(item: BridgeSupervisorWorkItem, interactionId: string): void {
    this.transition(item, "waiting_for_interaction", { interactionId })
  }

  recordInteractionAnswered(item: BridgeSupervisorWorkItem, interactionId: string): void {
  ...
  checkWatchdogs(): BridgeWatchdogResult[] {
    ...
    for (const turn of this.turns.values()) {
      ...
      if (now - lastProviderEventAt < this.providerSilentTimeoutMs) {
        continue
      }
      const item = { id: turn.queueItemId, claimId: turn.claimId, traceId: turn.traceId }
      this.recordFailed(item, "provider_silent_timeout")
      results.push({
        checkpoint: "failed",
  ...
  async publishHealthDiagnostic(input: {
    bridgeDeviceId?: string
    organizationId?: string
    traceId?: string
  } = {}): Promise<void> {
    ...
    const diagnostic: BridgeDiagnosticInput = {
      ...
      reasonCode:
        health.status === "healthy" ? "local_journal_healthy" : health.reasonCode,
  ...
  private persistPromptOutbox(item: BridgeSupervisorWorkItem): void {
    ...
    try {
      const { outboxId } = this.journal.recordClaimBeforePrompt({
        ...
        queueItemId: item.id,
        runtimeProfileId: item.runtimeProfileId ?? item.bridgeProfileId ?? item.hermesProfileName,
  ...
  private async publishRecoveryDiagnostic(row: RecoveryOutboxRow): Promise<void> {
    const diagnostic: BridgeDiagnosticInput = {
      details: {
        claimId: row.claimId,
      ...
      reasonCode: row.eventType === "prompt.send" ? "ambiguous_after_crash" : "outbox_replayed",
      traceId: row.traceId,
    }
    try {
      this.journal?.appendDiagnostic(diagnostic)
  ...
  private async tryAppendHostDiagnostic(diagnostic: BridgeDiagnosticInput): Promise<void> {
    try {
...
function isTerminalCheckpoint(checkpoint: BridgeTurnCheckpoint): boolean {
  return checkpoint === "cancelled" || checkpoint === "completed" || checkpoint === "failed"
}
