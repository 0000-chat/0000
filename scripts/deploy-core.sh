...
python3 scripts/render-synapse-config.py \
  ...
  --telegram-registration "$runtime_dir/synapse/telegram-registration.yaml" \
...
docker compose --env-file deploy/images.lock.env up -d --no-deps --force-recreate --wait --wait-timeout 180 telegram
