...
export async function seedAccountAccess(db: D1Database): Promise<void> {
  await db.batch([
    ...
    ).bind("gateway_route_human", "principal_operator", fixtureTimestamp, fixtureTimestamp),
    db.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
    ).bind("gateway_route_agent", "principal_operator", fixtureTimestamp, fixtureTimestamp),
    db.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
    ).bind("account_human", "connection_human_whatsapp", fixtureTimestamp, fixtureTimestamp),
    db.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
    ).bind("account_agent", "connection_agent_whatsapp", fixtureTimestamp, fixtureTimestamp),
    db.prepare(
    ...
    ).bind("grant_fixture_human", "tenant_pilot", "membership_human", "identity_human", "account_human", fixtureTimestamp, fixtureTimestamp),
    db.prepare(
      "INSERT INTO account_grants (id, tenant_id, membership_id, identity_id, account_id, operation_scope, chat_scope, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'conversation.read', 'all_chats', 'active', ?, ?)",
    ).bind("grant_fixture_agent", "tenant_pilot", "membership_agent", "identity_agent", "account_agent", fixtureTimestamp, fixtureTimestamp),
  ]);
}
