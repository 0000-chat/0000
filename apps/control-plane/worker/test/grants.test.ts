...
import { auth, bindingFor, event, initialize } from "./projection/projector-test-support";
...
const createTestApp = () => createApp({
  createTokenVerifier: () => ({
    verify: async (token: string): Promise<VerifiedSubject> => {
      if (token === "human-token") return { issuer: "https://issuer.example/", subject: "human-subject" };
      if (token === "agent-token") return { issuer: "https://issuer.example/", subject: "agent-subject", token_id: "agent-token-id" };
      if (token === "service-token") return { issuer: "https://issuer.example/", subject: "service-subject", token_id: "service-token-id" };
...
const request = async (
  ...
  token = "human-token",
  init: RequestInit = {},
) => createTestApp().request(`http://example.test${path}`, {
  ...init,
  headers: {
...
async function insertAccounts() {
  await workerEnv.CONTROL_DB.batch([
    ...
    ).bind("gateway_route_human", "principal_operator", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    workerEnv.CONTROL_DB.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
    ).bind("gateway_route_agent", "principal_operator", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    workerEnv.CONTROL_DB.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
    ).bind("gateway_route_agent_secondary", "principal_operator", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    workerEnv.CONTROL_DB.prepare(
    ...
    ).bind("connection_agent_secondary", tenantId, "identity_agent", "Agent Telegram", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    workerEnv.CONTROL_DB.prepare(
    ...
    ).bind("connection_agent_secondary", "gateway_route_agent_secondary", "bridge-agent-secondary", "route-user-agent-secondary", "route-room-agent-secondary", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    workerEnv.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
    ).bind("account_human", "connection_human_whatsapp", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    workerEnv.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
    ).bind("account_agent", "connection_agent_whatsapp", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    workerEnv.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
    ).bind("account_agent_secondary", "connection_agent_secondary", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
  ]);
}
...
async function projectReadFixtures() {
  ...
  await stub.applyBatch({
    ...
    mode: "live",
    rebuild_id: null,
    connections: [bindingFor("account_human", "connection_human_whatsapp", "identity_human")],
    events: [
      event("event_grant_human_one", {
...
beforeEach(async () => {
  ...
  await workerEnv.CONTROL_DB.batch([
    ...
    ).bind("principal_service", "https://issuer.example/", "service-subject", "Service", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    workerEnv.CONTROL_DB.prepare(
      "INSERT INTO memberships (id, tenant_id, principal_id, role, status, created_at, updated_at) VALUES (?, ?, ?, 'owner', 'active', ?, ?)",
    ).bind("membership_service", tenantId, "principal_service", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
  ]);
...
describe("account-scoped grant API", () => {
  it("fails closed for an ungranted linked account, then applies selected/all scope and revocation", async () => {
    ...
    expect(ConversationPageResultSchema.parse(await selected.json()).items.map((item) => item.id))
    ...
    await workerEnv.TENANT_PROJECTION.getByName(tenantId).applyBatch({
      ...
      rebuild_id: null,
      connections: [bindingFor("account_human", "connection_human_whatsapp", "identity_human")],
      events: [event("event_grant_human_future", {
    ...
    expect(ConversationPageResultSchema.parse(await allChats.json()).items.map((item) => item.id))
      .toEqual(["conversation_human_future", "conversation_human_two", "conversation_human_one"]);
    ...
    expect(new Set(audit.results.map((row) => row.target_id))).toEqual(new Set([grant.id]));
  });

  it("denies an agent grant, cross-account reads, and cross-tenant account binding", async () => {
    await workerEnv.CONTROL_DB.prepare("UPDATE memberships SET role = 'admin' WHERE id = ?")
      .bind("membership_agent").run();
    ...
    expect(ApiErrorResponseSchema.parse(await agentGrant.json()).error.code).toBe("forbidden");
    ...
    await workerEnv.CONTROL_DB.batch([
      ...
      ).bind("tenant_other", "other", "Other", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
      workerEnv.CONTROL_DB.prepare(
        "INSERT INTO principals (id, issuer, subject, principal_type, display_name, status, created_at, updated_at) VALUES (?, ?, ?, 'human', ?, 'active', ?, ?)",
      ).bind("principal_other", "https://issuer.example/", "other-subject", "Other", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
      workerEnv.CONTROL_DB.prepare(
        "INSERT INTO memberships (id, tenant_id, principal_id, role, status, created_at, updated_at) VALUES (?, ?, ?, 'owner', 'active', ?, ?)",
      ).bind("membership_other", "tenant_other", "principal_other", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
      workerEnv.CONTROL_DB.prepare(
        "INSERT INTO identities (id, tenant_id, identity_kind, display_name, status, created_at, updated_at) VALUES (?, ?, 'human', ?, 'active', ?, ?)",
      ).bind("identity_other", "tenant_other", "Other", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
      workerEnv.CONTROL_DB.prepare(
      ...
      ).bind("connection_other", "tenant_other", "identity_other", "Other WhatsApp", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
      workerEnv.CONTROL_DB.prepare(
        "INSERT INTO connection_routes (connection_id, gateway_route_id, bridge_instance_id, matrix_user_id, matrix_room_namespace, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
      ).bind("connection_other", "gateway_route_other", "bridge-other", "route-user-other", "route-room-other", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
      workerEnv.CONTROL_DB.prepare(
        "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
      ).bind("gateway_route_other", "principal_other", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
      workerEnv.CONTROL_DB.prepare(
        "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, 'active', ?, ?)",
      ).bind("account_other", "connection_other", "2026-09-07T00:00:00.000Z", "2026-09-07T00:00:00.000Z"),
    ]);
  ...
  it("filters delegated account and connection enumeration by active grants", async () => {
    ...
    expect(ConnectedAccountPageSchema.parse(await after.json()).items.map((account) => account.account_id))
    ...
    expect(ConnectionSchema.array().parse(await afterConnections.json()).map((connection) => connection.id))
  ...
  it("denies retained projection rows when the last account is retired or the account table is empty", async () => {
    ...
    ).bind("2026-09-07T04:00:00.000Z", "account_human").run();
  ...
  it("lets an agent inspect its own grants and request access without elevating itself", async () => {
    ...
    expect(await implicitOwnGrants.json()).toEqual({ items: [], next_cursor: null });
    ...
    const requestAccess = await request("/api/v1/permission-requests", "agent-token", {
      method: "POST",
      body: JSON.stringify({
        identity_id: "identity_agent",
        account_id: "account_agent",
    ...
    const otherIdentityRequest = await request("/api/v1/permission-requests", "agent-token", {
      method: "POST",
      body: JSON.stringify({
        identity_id: "identity_human",
        account_id: "account_human",
