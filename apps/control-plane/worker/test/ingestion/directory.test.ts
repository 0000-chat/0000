...
async function seedRouting(
  ...
  const routeRevokedAt =
    options.routeRevokedAt ?? (routeStatus === "revoked" ? timestamp : null);
  const serviceStatus = options.serviceStatus ?? "active";
  const serviceRevokedAt =
    options.serviceRevokedAt ??
    (serviceStatus === "revoked" ? timestamp : null);
  ...
  const retiredAt =
    options.retiredAt ?? (accountStatus === "retired" ? timestamp : null);
...
describe("ingestion control-directory repository", () => {
  it("finds only an active service principal and rejects revoked JWT IDs", async () => {
    ...
    await expect(
      findActiveIngestionService(
        db,
        issuer,
        "subject_service_revoked",
        "jti_revoked_service",
      ),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      findActiveIngestionService(db, issuer, "human_rejected", "jti_human"),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      findActiveIngestionService(db, issuer, "agent_rejected", "jti_agent"),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      findActiveIngestionService(
        db,
    ...
    await expect(
      findActiveIngestionService(db, issuer, "subject_service_active", ""),
    ).resolves.toEqual({ ok: false, code: "not_found" });
  });
  ...
  it("preserves unreferenced revoked principal tombstones against deletion and reinsertion", async () => {
    ...
    await expect(
      insert("DELETE FROM principals WHERE id = ?", principalId),
    ).rejects.toThrow();
    await expect(
      insert(
        "INSERT INTO principals (id, issuer, subject, principal_type, display_name, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    await expect(
      insert("DELETE FROM principals WHERE id = ?", deletablePrincipalId),
  ...
  it("blocks revoked principal identity rewrites and UPDATE OR REPLACE tombstone hijacks", async () => {
    ...
    const recursiveTriggers = await db
    ...
    await expect(
      db
        ...
        .bind(revokedId, activeId)
        .all(),
    ).resolves.toMatchObject({ results: beforeRows });
  ...
  it("resolves active routes only for their active service owner and archives inactive routes", async () => {
    ...
    const disabled = await seedRouting("route_disabled", {
      routeStatus: "disabled",
    });
    const revoked = await seedRouting("route_revoked", {
      routeStatus: "revoked",
    });
    const inactiveOwner = await seedRouting("route_inactive_owner", {
      serviceStatus: "disabled",
    });
    const otherActiveOwner = await seedRouting("route_other_active_owner");

    await expect(
      resolveActiveIngestionRoute(db, active.serviceId, active.routeId),
    ).resolves.toEqual({
      ok: true,
      value: {
        gateway_route_id: active.routeId,
        service_principal_id: active.serviceId,
      },
    });
    await expect(
      resolveActiveIngestionRoute(db, active.serviceId, disabled.routeId),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      resolveActiveIngestionRoute(
        db,
        active.serviceId,
        otherActiveOwner.routeId,
      ),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      resolveActiveIngestionRoute(
        db,
        inactiveOwner.serviceId,
        inactiveOwner.routeId,
      ),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      resolveActiveIngestionRoute(db, active.serviceId, revoked.routeId),
    ).resolves.toEqual({ ok: false, code: "not_found" });

    await expect(
      resolveArchivedIngestionRoute(db, disabled.routeId),
    ).resolves.toEqual({
      ok: true,
      value: {
        gateway_route_id: disabled.routeId,
        service_principal_id: disabled.serviceId,
      },
    });
    await expect(
      resolveArchivedIngestionRoute(db, revoked.routeId),
    ).resolves.toEqual({
      ok: true,
      value: {
        gateway_route_id: revoked.routeId,
        service_principal_id: revoked.serviceId,
      },
    });
    await expect(
      resolveArchivedIngestionRoute(db, inactiveOwner.routeId),
    ).resolves.toEqual({
      ok: true,
      value: {
        gateway_route_id: inactiveOwner.routeId,
        service_principal_id: inactiveOwner.serviceId,
      },
    });
  });

  it("returns active bindings sorted uniquely and accepts drainable connection states", async () => {
    const primary = await seedRouting("bindings_primary", {
    ...
    await expect(
      resolveActiveIngressBindings(db, primary.routeId, primary.tenantId, [
        "account_bindings_extra",
        primary.accountId,
      ]),
    ).resolves.toEqual({
  ...
  it("denies inactive ingress lifecycle states but lets archived replay resolve retired mappings", async () => {
    const retired = await seedRouting("bindings_retired", {
      accountStatus: "retired",
    });
    const inactiveTenant = await seedRouting("bindings_inactive_tenant", {
      tenantStatus: "disabled",
    });
    const inactiveIdentity = await seedRouting("bindings_inactive_identity", {
      identityStatus: "disabled",
    });
    const revokedConnection = await seedRouting("bindings_revoked_connection", {
      connectionStatus: "revoked",
    });
    const unlinkedConnection = await seedRouting(
      "bindings_unlinked_connection",
      { connectionStatus: "unlinked" },
    );

    await expect(
      resolveActiveIngressBindings(db, retired.routeId, retired.tenantId, [
        retired.accountId,
      ]),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      resolveActiveIngressBindings(
        db,
        inactiveTenant.routeId,
        inactiveTenant.tenantId,
        [inactiveTenant.accountId],
      ),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      resolveActiveIngressBindings(
        db,
        inactiveIdentity.routeId,
        inactiveIdentity.tenantId,
        [inactiveIdentity.accountId],
      ),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      resolveActiveIngressBindings(
        db,
        revokedConnection.routeId,
        revokedConnection.tenantId,
        [revokedConnection.accountId],
      ),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await expect(
      resolveActiveIngressBindings(
        db,
        unlinkedConnection.routeId,
        unlinkedConnection.tenantId,
        [unlinkedConnection.accountId],
      ),
    ).resolves.toEqual({ ok: false, code: "not_found" });

    await expect(
      resolveArchivedBindings(db, retired.routeId, retired.tenantId, [
        retired.accountId,
      ]),
    ).resolves.toMatchObject({
      ok: true,
      value: [{ account_id: retired.accountId, account_status: "retired" }],
    });
    await expect(
      resolveArchivedBindings(
        db,
  ...
  it("keeps two tenants and two routes isolated across WhatsApp, Telegram, and Messenger", async () => {
    ...
    const second = await seedRouting("tenant_two_telegram", {
      provider: "telegram",
    });
    await insert(
      "INSERT INTO connections (id, tenant_id, identity_id, provider, display_label, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    await expect(
      resolveActiveIngressBindings(db, first.routeId, first.tenantId, [
        first.accountId,
      ]),
    ...
    await expect(
      resolveActiveIngressBindings(db, second.routeId, second.tenantId, [
        second.accountId,
      ]),
    ).resolves.toMatchObject({ ok: true, value: [{ platform: "telegram" }] });
    await expect(
      resolveActiveIngressBindings(db, first.routeId, second.tenantId, [
        second.accountId,
      ]),
    ).resolves.toEqual({ ok: false, code: "not_found" });
  });
  ...
  it("maps D1 exceptions to a generic unavailable result and reads from first-primary", async () => {
    ...
    await expect(
      findActiveIngestionService(failureDb, issuer, "subject", "jti"),
  ...
  it("registers a legacy route explicitly before active ingestion can resolve it", async () => {
    ...
    await expect(
      resolveActiveIngressBindings(db, legacyRouteId, tenantId, [accountId]),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await insert(
    ...
    await expect(
      resolveActiveIngressBindings(db, routeId, tenantId, [accountId]),
    ).resolves.toMatchObject({
      ok: true,
      value: [{ account_id: accountId, gateway_route_id: routeId }],
    });
  });
  ...
  it("enforces immutable ownership and terminal lifecycle transitions while preserving replay evidence", async () => {
    ...
    const forbiddenUpdates = [
      [
        "UPDATE gateway_routes SET id = ? WHERE id = ?",
        "gateway_route_replaced",
        seeded.routeId,
      ],
      [
        "UPDATE gateway_routes SET service_principal_id = ? WHERE id = ?",
        "principal_service_other",
        seeded.routeId,
      ],
      [
        "UPDATE connection_accounts SET account_id = ? WHERE account_id = ?",
        "account_replaced",
        seeded.accountId,
      ],
      [
        "UPDATE connection_accounts SET connection_id = ? WHERE account_id = ?",
        "connection_replaced",
        seeded.accountId,
      ],
      [
        "UPDATE connections SET tenant_id = ? WHERE id = ?",
        "tenant_other",
        seeded.connectionId,
      ],
      [
        "UPDATE connections SET identity_id = ? WHERE id = ?",
        "identity_other",
        seeded.connectionId,
      ],
      [
        "UPDATE connections SET provider = ? WHERE id = ?",
        "telegram",
        seeded.connectionId,
      ],
      [
        "UPDATE principals SET issuer = ? WHERE id = ?",
        "https://other.example/",
        seeded.serviceId,
      ],
      [
        "UPDATE principals SET subject = ? WHERE id = ?",
        "subject_replaced",
        seeded.serviceId,
      ],
      [
        "UPDATE principals SET principal_type = ? WHERE id = ?",
        "agent",
        seeded.serviceId,
      ],
      [
        "UPDATE connection_routes SET connection_id = ? WHERE connection_id = ?",
        "connection_replaced",
        seeded.connectionId,
      ],
      [
    ...
    await expect(
      insert("DELETE FROM gateway_routes WHERE id = ?", seeded.routeId),
    ).rejects.toThrow();
    await expect(
      insert(
        "DELETE FROM connection_accounts WHERE account_id = ?",
        seeded.accountId,
      ),
    ).rejects.toThrow();
    await expect(
    ...
    await expect(
      insert(
        "UPDATE gateway_routes SET status = 'active', revoked_at = NULL WHERE id = ?",
        seeded.routeId,
      ),
    ).rejects.toThrow();
    await insert(
    ...
    await expect(
      insert(
        "UPDATE connection_accounts SET status = 'active', retired_at = NULL WHERE account_id = ?",
        seeded.accountId,
      ),
    ).rejects.toThrow();
    ...
    await expect(
      resolveArchivedIngestionRoute(db, seeded.routeId),
    ).resolves.toEqual({
      ok: true,
      value: {
        gateway_route_id: seeded.routeId,
        service_principal_id: seeded.serviceId,
      },
    });
    await expect(
      resolveArchivedBindings(db, seeded.routeId, seeded.tenantId, [
        seeded.accountId,
      ]),
    ).resolves.toMatchObject({
      ok: true,
  ...
  it("rejects UPDATE OR REPLACE from an unregistered legacy row into a registered connection", async () => {
    ...
    const beforeRoutes = (
      await db
        ...
        .bind(legacyConnectionId, registered.connectionId)
        .all()
    ).results;
    ...
    await expect(
      db
        ...
        .bind(legacyConnectionId, registered.connectionId)
        .all(),
    ).resolves.toMatchObject({ results: beforeRoutes });
    await expect(
      db
        .prepare(
          "SELECT account_id, connection_id, status, created_at, updated_at, retired_at FROM connection_accounts WHERE account_id = ?",
        ...
        .bind(registered.accountId)
        .all(),
    ).resolves.toMatchObject({ results: beforeAccounts });
    await expect(
      resolveActiveIngressBindings(
        db,
        registered.routeId,
      ...
      value: [
        {
          account_id: registered.accountId,
  ...
  it("rejects an unbounded account request before SQL and sanitizes binding failures", async () => {
    ...
    await expect(
      resolveArchivedBindings(failureDb, "gateway_route_one", "tenant_one", [
        "account_one",
      ]),
    ).resolves.toEqual({ ok: false, code: "unavailable" });
  });
  ...
  it("resolves a bounded request across query chunks and preserves UTF-8 account ordering", async () => {
      ...
      statements.push(
        ...
        db
          ...
          .bind(
            connectionId,
            seeded.routeId,
            `bridge_chunked_${suffix}`,
            `matrix_user_chunked_${suffix}`,
            `room_chunked_${suffix}`,
            timestamp,
            timestamp,
          ),
        db
    ...
    const result = await resolveActiveIngressBindings(
      db,
      seeded.routeId,
      seeded.tenantId,
      accountIds,
    ...
    if (result.ok) {
      ...
      expect(result.value.map((binding) => binding.account_id)).toEqual(
        Array.from(
          { length: count },
          (_, index) => `account_chunked_${String(index).padStart(3, "0")}`,
        ),
      );
    }
  });
  ...
  it("observes a revocation or disablement committed immediately before a lookup", async () => {
    ...
    await expect(
      resolveActiveIngestionRoute(db, seeded.serviceId, seeded.routeId),
    ).resolves.toEqual({ ok: false, code: "not_found" });
    await insert(
      "UPDATE principals SET status = 'revoked', revoked_at = ? WHERE id = ?",
      timestamp,
