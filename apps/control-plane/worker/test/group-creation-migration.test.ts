  ...
  await env.CONTROL_DB.batch([
    ...
    env.CONTROL_DB.prepare(
      `INSERT INTO removal_authority
       ...
       VALUES (?, ?, 'message', ?, ?, ?, ?, ?, ?, 'requested', ?, 4, 'active', 'not_started', ?, ?)`,
    ).bind(
      "removal_upgrade",
      "tenant_upgrade",
      "message_upgrade",
...
describe("group creation upgrade migration", () => {
  it("preserves populated grants and removal authority while adding group authority", async () => {
    const before = {
      identityGrants: await env.CONTROL_DB.prepare(
        "SELECT tenant_id, membership_id, identity_id, operation_scope, created_at FROM identity_grants WHERE tenant_id = ? ORDER BY operation_scope",
      )
        .bind("tenant_upgrade")
        .all(),
      accountGrants: await env.CONTROL_DB.prepare(
        "SELECT id, operation_scope, chat_scope, status, revoked_at FROM account_grants WHERE tenant_id = ? ORDER BY id",
      )
        .bind("tenant_upgrade")
        .all(),
      selectedChats: await env.CONTROL_DB.prepare(
      ...
      permission: await env.CONTROL_DB.prepare(
        ...
        .bind("tenant_upgrade")
        .all(),
      mutations: await env.CONTROL_DB.prepare(
      ...
      removal: await env.CONTROL_DB.prepare(
        "SELECT id, resource_type, resource_id, content_generation, deletion_epoch, status, purge_status FROM removal_authority WHERE tenant_id = ?",
      )
        .bind("tenant_upgrade")
        .all(),
      expiry: await env.CONTROL_DB.prepare(
        "SELECT id, resource_type, resource_id, content_generation, status, removal_id FROM removal_expiry_schedule WHERE tenant_id = ?",
      )
        .bind("tenant_upgrade")
        .all(),
    };
    ...
    await applyD1Migrations(env.CONTROL_DB, [
      migration("0019_group_creation.sql"),
    ]);
    ...
    expect(after.identityGrants.results).toEqual(before.identityGrants.results);
    expect(after.accountGrants.results).toEqual(before.accountGrants.results);
    expect(after.selectedChats.results).toEqual(before.selectedChats.results);
    expect(after.permission.results).toEqual(before.permission.results);
    expect(after.mutations.results).toEqual(before.mutations.results);
    expect(after.removal.results).toEqual(before.removal.results);
    expect(after.expiry.results).toEqual(before.expiry.results);

    expect(
      (await env.CONTROL_DB.prepare("PRAGMA foreign_key_check").all()).results,
    ).toEqual([]);
    expect(
      await env.CONTROL_DB.prepare("PRAGMA foreign_keys").first<{
        foreign_keys: number;
      }>(),
    ...
    await env.CONTROL_DB.batch([
      ...
      env.CONTROL_DB.prepare(
        "INSERT INTO account_grants (id, tenant_id, membership_id, identity_id, account_id, operation_scope, chat_scope, status, created_at, updated_at, revoked_at) VALUES (?, ?, ?, ?, ?, 'group.create', 'all_chats', 'active', ?, ?, NULL)",
      ).bind(
        "grant_upgrade_group",
        "tenant_upgrade",
        "membership_upgrade",
    ...
    const tables = await env.CONTROL_DB.prepare(
      "SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('group_creation_operations', 'group_creation_access_grants', 'group_creation_webhook_evaluations') ORDER BY name",
    ).all<{ name: string }>();
