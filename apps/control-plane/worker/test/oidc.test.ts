...
describe("createOidcVerifier", () => {
  ...
  it("preserves an injected unavailable key-resolution failure", async () => {
    const unavailable = createOidcVerifier(
      oidcConfig,
      async () => {
  ...
  it.each([
    ...
    ["an empty key set", async () => new Response(JSON.stringify({ keys: [] }), { status: 200 })],
    ...
    await expect(remote.verify(token)).rejects.toMatchObject({ code: "unavailable" });
  });

  it("classifies an unknown kid as invalid after a usable JWKS loads", async () => {
    const publicJwk = await exportJWK(await generateKeyPair("ES256").then(({ publicKey }) => publicKey));
    const fetch: FetchImplementation = async () => new Response(JSON.stringify({
      keys: [{ ...publicJwk, alg: "ES256", kid: "different-key" }],
    }), { status: 200 });
    const remote = createOidcVerifier(oidcConfig, undefined, { fetch });
    const token = await signTestToken(privateKey, { token_id: "remote-token", kid: "missing-key" });

    await expect(remote.verify(token)).rejects.toMatchObject({ code: "invalid" });
  });

  it.each([
    ["a bad signature", async () => {
      const other = await generateKeyPair("ES256");
      return signTestToken(other.privateKey, { token_id: "remote-bad-signature" });
    }],
    ["a wrong issuer", () => signTestToken(privateKey, {
      issuer: "https://other.example/",
      token_id: "remote-wrong-issuer",
    })],
    ["a wrong audience", () => signTestToken(privateKey, {
      audience: "other-api",
      token_id: "remote-wrong-audience",
    })],
    ["a malformed token", async () => "not-a-token"],
  ])("classifies %s as invalid with a remote usable JWKS", async (_name, makeToken) => {
    const fetch: FetchImplementation = async () => new Response(JSON.stringify({
      keys: [{ ...publicJwk, alg: "ES256", kid: "local-key" }],
    }), { status: 200 });
    const remote = createOidcVerifier(oidcConfig, undefined, { fetch });

    await expect(remote.verify(await makeToken())).rejects.toMatchObject({ code: "invalid" });
  });
  ...
  it("classifies a disallowed algorithm as invalid with a remote usable JWKS", async () => {
    ...
    const remote = createOidcVerifier(oidcConfig, undefined, { fetch });
    const signed = await signTestToken(privateKey, { token_id: "remote-algorithm" });
    ...
    await expect(remote.verify(`${header}.${encodedPayload}.${signature}`)).rejects.toMatchObject({
      code: "invalid",
    });
  ...
  it("keeps token failures invalid after a previous remote fetch outage", async () => {
    ...
    const remote = createOidcVerifier(oidcConfig, undefined, { fetch });
    const valid = await signTestToken(privateKey, { token_id: "remote-recovery" });
    await expect(remote.verify(valid)).rejects.toMatchObject({ code: "unavailable" });

    const other = await generateKeyPair("ES256");
    const badSignature = await signTestToken(other.privateKey, { token_id: "remote-recovery" });
    await expect(remote.verify(badSignature)).rejects.toMatchObject({ code: "invalid" });
    expect(calls).toBe(2);
  });
  ...
  it("treats an expired ingestion token as invalid after a remote usable JWKS loads", async () => {
    ...
    await expect(remote.verify(expired)).rejects.toMatchObject({ code: "invalid" });
  });

  it.each([
    ["a P-384 key labelled ES256", async () => {
      const pair = await generateKeyPair("ES384");
      const jwk = await exportJWK(pair.publicKey);
      return { ...jwk, alg: "ES256", kid: "local-key" };
    }],
    ["a malformed P-256 key", async () => ({
      ...publicJwk,
      alg: "ES256",
      kid: "local-key",
      x: "not-base64url",
    })],
    ["a key with duplicate operations", async () => ({
      ...publicJwk,
      alg: "ES256",
      kid: "local-key",
