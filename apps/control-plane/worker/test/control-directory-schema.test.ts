...
async function dropControlDirectorySchema(db: D1Database) {
  ...
  for (const { name } of triggers.results) {
    if (!/^[A-Za-z0-9_]+$/.test(name)) throw new Error("unexpected trigger name");
...
describe("control directory schema", () => {
  ...
  it("creates bounded ingestion routing tables, indexes, and lifecycle triggers", async () => {
    ...
    expect(tableInfo.results.find((row) => row.name === "gateway_routes")?.sql).toContain(
      "status IN ('active', 'disabled', 'revoked')",
    );
    expect(tableInfo.results.find((row) => row.name === "connection_accounts")?.sql).toContain(
  ...
  it("enforces ingestion routing checks and foreign-key ownership", async () => {
    ...
    ).bind("gateway_route_missing_principal", "principal_missing", "active", timestamp, timestamp).run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at, revoked_at) VALUES (?, ?, ?, ?, ?, ?)",
    ).bind("gateway_route_bad_status", "principal_missing", "paused", timestamp, timestamp, null).run()).rejects.toThrow();

    await seedDirectory(env.CONTROL_DB);
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at, revoked_at) VALUES (?, ?, ?, ?, ?, ?)",
    ).bind("gateway_route_check", "principal_human", "revoked", timestamp, timestamp, null).run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_missing_connection", "connection_missing", "active", timestamp, timestamp).run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_bad_status", "connection_human_whatsapp", "paused", timestamp, timestamp).run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at, retired_at) VALUES (?, ?, ?, ?, ?, ?)",
    ).bind("account_bad_retired_at", "connection_human_whatsapp", "active", timestamp, timestamp, timestamp).run()).rejects.toThrow();
  });
  ...
  it("freezes ownership and terminal history once an account is registered", async () => {
    ...
    await env.CONTROL_DB.batch([
      env.CONTROL_DB.prepare(
        "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
      ).bind("gateway_route_human", "principal_human", "active", timestamp, timestamp),
      env.CONTROL_DB.prepare(
        "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
      ).bind("account_human_whatsapp", "connection_human_whatsapp", "active", timestamp, timestamp),
    ]);

    for (const [sql, value] of [
      ["UPDATE gateway_routes SET id = ? WHERE id = 'gateway_route_human'", "gateway_route_other"],
      ["UPDATE gateway_routes SET service_principal_id = ? WHERE id = 'gateway_route_human'", "principal_agent"],
      ["UPDATE connection_accounts SET account_id = ? WHERE account_id = 'account_human_whatsapp'", "account_other"],
      ["UPDATE connection_accounts SET connection_id = ? WHERE account_id = 'account_human_whatsapp'", "connection_other"],
      ["UPDATE connections SET tenant_id = ? WHERE id = 'connection_human_whatsapp'", "tenant_other"],
      ["UPDATE connections SET identity_id = ? WHERE id = 'connection_human_whatsapp'", "identity_agent"],
      ["UPDATE connections SET provider = ? WHERE id = 'connection_human_whatsapp'", "telegram"],
      ["UPDATE principals SET issuer = ? WHERE id = 'principal_human'", "https://other.example/"],
      ["UPDATE principals SET subject = ? WHERE id = 'principal_human'", "other-subject"],
      ["UPDATE principals SET principal_type = ? WHERE id = 'principal_human'", "agent"],
      ["UPDATE connection_routes SET connection_id = ? WHERE connection_id = 'connection_human_whatsapp'", "connection_other"],
      ["UPDATE connection_routes SET gateway_route_id = ? WHERE connection_id = 'connection_human_whatsapp'", "gateway_route_other"],
    ] as const) {
      await expect(env.CONTROL_DB.prepare(sql).bind(value).run()).rejects.toThrow();
    }
    await expect(env.CONTROL_DB.prepare("DELETE FROM gateway_routes WHERE id = 'gateway_route_human'").run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare("DELETE FROM connection_accounts WHERE account_id = 'account_human_whatsapp'").run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare("DELETE FROM connection_routes WHERE connection_id = 'connection_human_whatsapp'").run()).rejects.toThrow();

    await env.CONTROL_DB.prepare(
      "UPDATE principals SET status = 'revoked', revoked_at = ? WHERE id = 'principal_human'",
    ).bind(timestamp).run();
    await expect(env.CONTROL_DB.prepare("UPDATE principals SET status = 'active' WHERE id = 'principal_human'").run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare("UPDATE principals SET revoked_at = NULL WHERE id = 'principal_human'").run()).rejects.toThrow();

    await env.CONTROL_DB.prepare(
      "INSERT INTO revoked_tokens (issuer, token_id, principal_id, reason, revoked_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("https://issuer.example/", "jti_append_only", "principal_human", "security review", timestamp).run();
    await expect(env.CONTROL_DB.prepare("UPDATE revoked_tokens SET reason = ? WHERE token_id = 'jti_append_only'").bind("changed").run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare("DELETE FROM revoked_tokens WHERE token_id = 'jti_append_only'").run()).rejects.toThrow();

    await env.CONTROL_DB.prepare(
      "UPDATE gateway_routes SET status = 'revoked', revoked_at = ? WHERE id = 'gateway_route_human'",
    ).bind(timestamp).run();
    await expect(env.CONTROL_DB.prepare("UPDATE gateway_routes SET status = 'active', revoked_at = NULL WHERE id = 'gateway_route_human'").run()).rejects.toThrow();
    await env.CONTROL_DB.prepare(
      "UPDATE connection_accounts SET status = 'retired', retired_at = ? WHERE account_id = 'account_human_whatsapp'",
    ).bind(timestamp).run();
    await expect(env.CONTROL_DB.prepare("UPDATE connection_accounts SET status = 'active', retired_at = NULL WHERE account_id = 'account_human_whatsapp'").run()).rejects.toThrow();
  });
  ...
  it("rejects INSERT OR REPLACE hijacks while recursive triggers remain disabled", async () => {
    ...
    await insertPrincipal("principal_replace_route", "https://replace.example/", "route");
    await env.CONTROL_DB.prepare(
      "UPDATE principals SET principal_type = 'service' WHERE id = 'principal_replace_route'",
    ).run();
    await insertPrincipal("principal_replace_route_other", "https://replace.example/", "route-other");
    await env.CONTROL_DB.prepare(
      "UPDATE principals SET principal_type = 'service' WHERE id = 'principal_replace_route_other'",
    ).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("gateway_route_replace", "principal_replace_route", "active", timestamp, timestamp).run();

    await expect(env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("gateway_route_replace", "principal_replace_route_other", "active", timestamp, timestamp).run()).rejects.toThrow();
    ...
    await env.CONTROL_DB.batch([
      env.CONTROL_DB.prepare(
        "INSERT INTO connections (id, tenant_id, identity_id, provider, display_label, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      ).bind("connection_replace_one", "tenant_replace", "identity_replace_one", "whatsapp", "One", "ready", timestamp, timestamp),
      env.CONTROL_DB.prepare(
        "INSERT INTO connections (id, tenant_id, identity_id, provider, display_label, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      ).bind("connection_replace_two", "tenant_replace", "identity_replace_two", "telegram", "Two", "ready", timestamp, timestamp),
      env.CONTROL_DB.prepare(
      ...
      ).bind("connection_replace_one", "gateway_route_replace", "bridge-one", "user-one", "room-one", timestamp, timestamp),
      env.CONTROL_DB.prepare(
        "INSERT INTO connection_routes (connection_id, gateway_route_id, bridge_instance_id, matrix_user_id, matrix_room_namespace, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
      ).bind("connection_replace_two", "gateway_route_replace", "bridge-two", "user-two", "room-two", timestamp, timestamp),
      env.CONTROL_DB.prepare(
        "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
      ).bind("account_replace_one", "connection_replace_one", "active", timestamp, timestamp),
      env.CONTROL_DB.prepare(
        "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
      ).bind("account_replace_two", "connection_replace_two", "active", timestamp, timestamp),
    ]);

    await expect(env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_replace_one", "connection_replace_two", "active", timestamp, timestamp).run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_replace_other", "connection_replace_one", "active", timestamp, timestamp).run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
    ...
    ).bind("connection_replace_one", "gateway_route_other", "bridge-hijack", "user-hijack", "room-hijack", timestamp, timestamp).run()).rejects.toThrow();

    await insertPrincipal("principal_replace_token", "https://replace.example/", "token");
    await env.CONTROL_DB.prepare(
      "INSERT INTO revoked_tokens (issuer, token_id, principal_id, reason, revoked_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("https://replace.example/", "jti_replace", "principal_replace_token", "security review", timestamp).run();
    await expect(env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO revoked_tokens (issuer, token_id, principal_id, reason, revoked_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("https://replace.example/", "jti_replace", "principal_replace_route", "replacement", timestamp).run()).rejects.toThrow();

    await insertPrincipal("principal_replace_revoked", "https://replace.example/", "revoked");
    await env.CONTROL_DB.prepare(
    ...
    ).bind("principal_replace_revoked", "https://replace.example/", "replacement", "service", "Replacement", "active", timestamp, timestamp).run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
    ...
    ).bind("principal_replace_revoked_other", "https://replace.example/", "revoked", "service", "Replacement", "active", timestamp, timestamp, null).run()).rejects.toThrow();
  });
  ...
  it("keeps a pre-migration legacy route unregistered until explicit registration", async () => {
    ...
    await env.CONTROL_DB.prepare(
      "INSERT INTO connections (id, tenant_id, identity_id, provider, display_label, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ).bind("connection_legacy", "tenant_legacy", "identity_legacy", "whatsapp", "Legacy", "ready", timestamp, timestamp).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO connection_routes (connection_id, gateway_route_id, bridge_instance_id, matrix_user_id, matrix_room_namespace, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ).bind("connection_legacy", "gateway-legacy", "bridge", "user", "room", timestamp, timestamp).run();

    await expect(env.CONTROL_DB.prepare(
      "SELECT gateway_route_id FROM connection_routes WHERE connection_id = ?",
    ).bind("connection_legacy").first()).resolves.toMatchObject({ gateway_route_id: "gateway-legacy" });
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_legacy", "connection_legacy", "active", timestamp, timestamp).run()).rejects.toThrow();
    await insertPrincipal("principal_legacy_service", "https://legacy.example/", "legacy-service");
    await env.CONTROL_DB.prepare(
    ...
    ).bind("gateway_route_legacy", "connection_legacy").run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("gateway_route_legacy", "principal_legacy_service", "active", timestamp, timestamp).run();
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_legacy", "connection_legacy", "active", timestamp, timestamp).run()).resolves.toBeDefined();
    await expect(env.CONTROL_DB.prepare(
      "UPDATE connection_routes SET gateway_route_id = ? WHERE connection_id = ?",
    ).bind("gateway_route_legacy_other", "connection_legacy").run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
      "INSERT OR REPLACE INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_legacy", "connection_legacy", "active", timestamp, timestamp).run()).rejects.toThrow();
  });
  ...
  it("migrates a real 0001-only legacy database and also boots a fresh 0001+0002 database", async () => {
    const migrationOne = env.TEST_MIGRATIONS.find((migration) => migration.name === "0001_control_directory.sql");
    const migrationTwo = env.TEST_MIGRATIONS.find((migration) => migration.name === "0002_ingestion_routing.sql");
    expect(migrationOne).toBeDefined();
    expect(migrationTwo).toBeDefined();
    if (!migrationOne || !migrationTwo) throw new Error("expected control-directory migrations");
    ...
    await insertPrincipal("principal_migration_legacy", "https://migration.example/", "legacy");
    await env.CONTROL_DB.prepare(
      "UPDATE principals SET principal_type = 'service' WHERE id = 'principal_migration_legacy'",
    ).run();
    await insertIdentity("identity_migration_legacy", "tenant_migration_legacy");
    await env.CONTROL_DB.prepare(
      "INSERT INTO connections (id, tenant_id, identity_id, provider, display_label, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ).bind("connection_migration_legacy", "tenant_migration_legacy", "identity_migration_legacy", "telegram", "Legacy", "ready", timestamp, timestamp).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO connection_routes (connection_id, gateway_route_id, bridge_instance_id, matrix_user_id, matrix_room_namespace, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ).bind("connection_migration_legacy", "gateway-legacy-migration", "bridge", "user", "room", timestamp, timestamp).run();

    await applyD1Migrations(env.CONTROL_DB, [migrationTwo]);
    await expect(env.CONTROL_DB.prepare(
      "SELECT gateway_route_id FROM connection_routes WHERE connection_id = ?",
    ).bind("connection_migration_legacy").first()).resolves.toEqual({
      gateway_route_id: "gateway-legacy-migration",
    });
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_migration_legacy", "connection_migration_legacy", "active", timestamp, timestamp).run()).rejects.toThrow();

    await env.CONTROL_DB.prepare(
      "UPDATE connection_routes SET gateway_route_id = ? WHERE connection_id = ?",
    ).bind("gateway_route_migration_legacy", "connection_migration_legacy").run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("gateway_route_migration_legacy", "principal_migration_legacy", "active", timestamp, timestamp).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_migration_legacy", "connection_migration_legacy", "active", timestamp, timestamp).run();
    await expect(env.CONTROL_DB.prepare(
      "UPDATE connection_routes SET gateway_route_id = ? WHERE connection_id = ?",
    ).bind("gateway_route_migration_other", "connection_migration_legacy").run()).rejects.toThrow();
    await expect(env.CONTROL_DB.prepare(
    ...
    await insertPrincipal("principal_migration_fresh", "https://migration.example/", "fresh");
    await env.CONTROL_DB.prepare(
      "UPDATE principals SET principal_type = 'service' WHERE id = 'principal_migration_fresh'",
    ).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO gateway_routes (id, service_principal_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("gateway_route_migration_fresh", "principal_migration_fresh", "active", timestamp, timestamp).run();
    await insertIdentity("identity_migration_fresh", "tenant_migration_fresh");
    await env.CONTROL_DB.prepare(
      "INSERT INTO connections (id, tenant_id, identity_id, provider, display_label, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ).bind("connection_migration_fresh", "tenant_migration_fresh", "identity_migration_fresh", "whatsapp", "Fresh", "ready", timestamp, timestamp).run();
    await env.CONTROL_DB.prepare(
      "INSERT INTO connection_routes (connection_id, gateway_route_id, bridge_instance_id, matrix_user_id, matrix_room_namespace, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ).bind("connection_migration_fresh", "gateway_route_migration_fresh", "bridge", "user", "room", timestamp, timestamp).run();
    await expect(env.CONTROL_DB.prepare(
      "INSERT INTO connection_accounts (account_id, connection_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
    ).bind("account_migration_fresh", "connection_migration_fresh", "active", timestamp, timestamp).run()).resolves.toBeDefined();
  });
});
