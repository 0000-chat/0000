...
const canonicalChatIds = (chatIds: readonly string[]): string[] => {
  const values = [...new Set(chatIds)].sort((left, right) => left.localeCompare(right));
...
const mapAccountGrantRows = (
  ...
  return rows.map((row) => AccountGrantSchema.parse({
    id: row.id,
...
): PermissionRequest[] => rows.map((row) => PermissionRequestSchema.parse({
  id: row.id,
  tenant_id: row.tenant_id,
...
export async function listAccountGrants(
  ...
  try {
    result = await db.prepare(query).bind(...bindings).all<AccountGrantRow>();
    const visible = result.results.slice(0, limit);
    const next = result.results.length > limit ? visible.at(-1)?.id ?? null : null;
    if (visible.length === 0) return AccountGrantPageSchema.parse({ items: [], next_cursor: null });
    const placeholders = visible.map(() => "?").join(",");
    const chatRows = await db.prepare(
    ...
    ).bind(input.tenantId, ...visible.map((grant) => grant.id)).all<AccountGrantChatRow>();
...
export async function getAccountGrant(
  ...
  try {
    const row = await db.prepare(`${grantRowsQuery} AND g.id = ? LIMIT 1`)
    ...
    ).bind(tenantId, grantId).all<AccountGrantChatRow>();
...
const mutationRecordStatement = async (
  ...
  ).bind(idempotencyKey, tenantId, actorPrincipalId, mutationType, requestHash, occurredAt);
};

const grantAuditStatements = (
  db: D1Database,
  input: { idempotencyKey: string; tenantId: string; actorPrincipalId: string; action: string; grantId: string; payload: unknown; occurredAt: string },
) => {
...
const grantChatStatements = (
  ...
  db.prepare("DELETE FROM account_grant_chats WHERE tenant_id = ? AND grant_id = ?")
    .bind(tenantId, grantId),
  ...(chatScope === "selected_chats"
    ? canonicalChatIds(chatIds).map((chatId) => db.prepare(
...
export async function createAccountGrant(
  ...
  try {
    ...
    await db.batch([
      ...
      ...grantChatStatements(db, grantId, input.tenantId, input.accountId, input.chatScope, chatIds, input.occurredAt),
    ...
    return await getAccountGrant(db.withSession("first-primary"), input.tenantId, grantId);
  } catch (error) {
    if (error instanceof GrantRepositoryError) throw error;
...
export async function updateAccountGrant(
  ...
  try {
    const existing = await getAccountGrant(db.withSession("first-primary"), input.tenantId, input.grantId);
    const mutation = await mutationRecordStatement(db, input.idempotencyKey, input.tenantId, input.actorPrincipalId, "authorization.account_grant.updated", payload, input.occurredAt);
    await db.batch([
      mutation,
      db.prepare(
        "UPDATE account_grants SET operation_scope = ?, chat_scope = ?, status = 'active', updated_at = ?, revoked_at = NULL WHERE tenant_id = ? AND id = ?",
      ).bind(input.operationScope, input.chatScope, input.occurredAt, input.tenantId, input.grantId),
      ...grantChatStatements(db, input.grantId, input.tenantId, existing.account_id, input.chatScope, chatIds, input.occurredAt),
      ...grantAuditStatements(db, {
        idempotencyKey: input.idempotencyKey,
        tenantId: input.tenantId,
        actorPrincipalId: input.actorPrincipalId,
    ...
    return await getAccountGrant(db.withSession("first-primary"), input.tenantId, input.grantId);
  } catch (error) {
    if (error instanceof GrantRepositoryError) throw error;
    throw grantError("grant_conflict", error);
  }
...
export async function revokeAccountGrant(
  ...
  const payload = { tenant_id: input.tenantId, grant_id: input.grantId, status: "revoked" };
  try {
    await getAccountGrant(db.withSession("first-primary"), input.tenantId, input.grantId);
    const mutation = await mutationRecordStatement(db, input.idempotencyKey, input.tenantId, input.actorPrincipalId, "authorization.account_grant.revoked", payload, input.occurredAt);
    await db.batch([
      mutation,
      db.prepare(
        "UPDATE account_grants SET status = 'revoked', revoked_at = ?, updated_at = ? WHERE tenant_id = ? AND id = ?",
      ).bind(input.occurredAt, input.occurredAt, input.tenantId, input.grantId),
      ...grantAuditStatements(db, {
        idempotencyKey: input.idempotencyKey,
        tenantId: input.tenantId,
        actorPrincipalId: input.actorPrincipalId,
...
export async function listConnectedAccounts(
  ...
  try {
    ...
    ).bind(...bindings).all<ConnectedAccount>();
    const visible = result.results.slice(0, limit).map((row) => ConnectedAccountSchema.parse({
      account_id: row.account_id,
      tenant_id: row.tenant_id,
      connection_id: row.connection_id,
      identity_id: row.identity_id,
    ...
    return ConnectedAccountPageSchema.parse({
      items: visible,
      next_cursor: result.results.length > limit ? visible.at(-1)?.account_id ?? null : null,
    });
  } catch (error) {
    throw grantError("grant_unavailable", error);
  }
...
export async function createPermissionRequest(
  ...
  const chatIds = validateChatScope(input.chatScope, input.chatIds);
  const requestId = input.requestId ?? `request_${(await sha256Hex(input.idempotencyKey)).slice(0, 48)}`;
  const payload = {
    tenant_id: input.tenantId,
  ...
  try {
    const mutation = await mutationRecordStatement(db, input.idempotencyKey, input.tenantId, input.requesterPrincipalId, "authorization.permission_request.created", payload, input.occurredAt);
    await db.batch([
      mutation,
      db.prepare(
    ...
    ).bind(input.tenantId, requestId).first<PermissionRequestRow>();
    if (!row) throw grantError("grant_not_found");
