...
export async function toGrantedAccountReadAuthorization(
  ...
  accountId: string,
): Promise<{ authorization: ProjectionAuthorizationContext; resourceIdentityId: string }> {
  const administrator = isAdministratorSession(session);
  const target = session.identities.find((identity) => identity.identity_id === targetIdentityId);
  ...
  try {
    ...
    const account = await db.prepare(
      `SELECT c.identity_id
       ...
       LIMIT 1`,
    ).bind(session.tenant.id, targetIdentityId, accountId).first<{ identity_id: string }>();
    ...
    const scope = administrator && targetIdentityId !== account.identity_id
      ? {
