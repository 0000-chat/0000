...
test("keeps Human and Agent inboxes isolated and denies a direct cross-identity route", async ({
  page,
}) => {
  await page.goto(
    `/conversations/${humanConversation}?identity=identity_human&channel=connection_human_telegram`,
  );
  await expect(
    page.getByRole("heading", { name: "Alex Rivera", level: 1 }),
  ).toBeVisible();
  await expect(
    page.getByTestId("message-viewport").getByText("I sent the outline"),
  ).toBeVisible();
  ...
  await page.goto(
    `/conversations/${humanConversation}?identity=identity_agent&channel=connection_agent_whatsapp`,
  );
...
test("filters channels without changing the conversation recency order", async ({
  page,
}) => {
  ...
  expect(
    await rows.evaluateAll((items) =>
      items.map((item) => item.getAttribute("data-conversation-id")),
    ),
  ).toEqual([
...
test("prepends older messages chronologically while keeping the composer fixed", async ({
  page,
}) => {
  const resetStatus = await page.evaluate(async () => {
    const response = await fetch("/api/v1/testing/reset", {
  ...
  const agentConversationLink = page.getByRole("link", {
  ...
  await expect(timeline).toContainText(
    "This fixture contains no live provider data.",
  );
  ...
  await expect(timeline).toContainText(
    "A simulated Agent conversation is available.",
  );
  await expect(timeline.locator("li").first()).toContainText(
    "A simulated Agent conversation is available.",
  );
  await expect(timeline.locator("li").last()).toContainText(
    "This fixture contains no live provider data.",
  );
  await expect(
...
test("shows bounded retry UI for a failed message read without rendering the payload", async ({
  page,
}) => {
  const resetStatus = await page.evaluate(async () => {
    const response = await fetch("/api/v1/testing/reset", {
  ...
  await page
    .locator(
      `[data-testid="conversation-row"][data-conversation-id="${humanConversation}"]`,
    )
    .click();
