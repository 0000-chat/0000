...
const targetValue = (target: Pick<AccountGrantTarget, "membership_id" | "identity_id">) =>
...
function ChatPicker({
  ...
  if (query.isLoading) return <p role="status" className="text-sm text-muted-foreground">Loading chats…</p>;
  if (query.isError) {
    return (
  ...
  return (
    <div className="space-y-2">
      {chats.length === 0 && <p className="text-sm text-muted-foreground">No chats are available for this account.</p>}
      {chats.map((chat) => (
        <label key={chat.id} className="flex items-start gap-2 rounded-md border p-2 text-sm">
          <input
            type="checkbox"
          ...
          <span className="min-w-0">
            ...
            <span className="block truncate text-xs text-muted-foreground">{chat.id}</span>
          </span>
        </label>
      ))}
      {query.hasNextPage && (
      ...
      {query.isFetchNextPageError && (
        <div role="alert" className="space-y-2 text-sm text-destructive">
          ...
          <Button type="button" size="sm" variant="outline" onClick={() => void query.fetchNextPage()}>Retry</Button>
        </div>
      )}
    </div>
  );
...
function GrantManagementPanel({
  ...
  const selectedTarget = targets.find((target) => targetValue(target) === targetKey) ?? targets[0];
  const narrowGrant = grants.find((grant) => grant.id === narrowGrantId);
  const pickerIdentityId = narrowGrant?.identity_id ?? selectedTarget?.identity_id;
  const pickerAccountId = narrowGrant?.account_id ?? accountId;
  const pickerSelectedChatIds = narrowGrantId === null ? chatIds : narrowChatIds;
  ...
  const chatsQuery = useInfiniteQuery<ConversationPageResult, Error>({
    ...
    queryFn: ({ pageParam }) => apiClient.getAccountConversations(
      pickerAccountId,
      pickerIdentityId ?? "",
  ...
  const createGrant = useMutation({
    mutationFn: () => {
      if (!selectedTarget || !accountId) throw new Error("Choose a target and account");
  ...
  const toggleChat = (chatId: string) => {
    ...
    setter((current) => current.includes(chatId)
    ...
    <section aria-labelledby="grant-management-heading" className="rounded-xl border bg-card p-5 shadow-sm">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Grant each eligible identity independent read, send, and webhook access for every account or selected chats.
        </p>
      </div>

      {targets.length === 0 && <p role="status" className="mt-4 text-sm text-muted-foreground">No eligible grant targets are available.</p>}
      <form
      ...
      >
        <label className="grid gap-1 text-sm font-medium">
          Identity
          <select className="h-9 rounded-md border bg-background px-2" value={selectedTarget ? targetValue(selectedTarget) : ""} onChange={(event) => setTargetKey(event.target.value)} disabled={targets.length === 0}>
            {targets.map((target) => (
              <option key={targetValue(target)} value={targetValue(target)}>
                {target.identity_display_name} · {target.principal_display_name} ({target.principal_type})
              </option>
            ))}
          </select>
        </label>
        <label className="grid gap-1 text-sm font-medium">
          Connected account
          <select className="h-9 rounded-md border bg-background px-2" value={accountId} onChange={(event) => setAccountId(event.target.value)} disabled={accounts.length === 0}>
        ...
        <div className="flex items-end">
          <Button type="submit" className="w-full" disabled={!selectedTarget || !accountId || createGrant.isPending}>
      ...
      {chatScope === "selected_chats" && (
        <div className="mt-3 grid gap-2 rounded-lg border p-3 sm:col-span-2">
          <p className="text-sm font-medium">Chats for selected identity and account</p>
          <ChatPicker chats={chats} selectedChatIds={pickerSelectedChatIds} onToggle={toggleChat} query={chatsQuery} />
          <p className="text-xs text-muted-foreground">{chatIds.length} chat{chatIds.length === 1 ? "" : "s"} selected.</p>
        </div>
      )}
          ...
          <div key={grant.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3 text-sm">
            ...
            {grant.status === "active" && narrowGrantId === grant.id && (
              <div className="basis-full space-y-2 rounded-lg border p-3">
                <p className="text-sm font-medium">Select chats for narrowed access</p>
                <ChatPicker chats={chats} selectedChatIds={narrowChatIds} onToggle={toggleChat} query={chatsQuery} />
                <Button
                  type="button"
                  size="sm"
                  disabled={updateGrant.isPending || narrowChatIds.length === 0}
                  onClick={() => updateGrant.mutate({ grant, ids: narrowChatIds })}
                >
      ...
      {accountsHasNextPage && (
        <Button type="button" variant="outline" className="mt-4" disabled={accountsFetchingNextPage} onClick={onLoadMoreAccounts}>
          {accountsFetchingNextPage ? "Loading more accounts…" : "Load more accounts"}
        </Button>
      )}
      {grantsHasNextPage && (
        <Button type="button" variant="outline" className="ml-2 mt-4" disabled={grantsFetchingNextPage} onClick={onLoadMoreGrants}>
...
export function ConnectionsPage() {
  ...
  const { activeIdentity, session, isLoading: identitiesLoading } = useIdentityContext();
  ...
  const accountsQuery = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => apiClient.getConnectedAccounts(
      undefined,
      typeof pageParam === "string" ? pageParam : undefined,
      2,
    ),
  ...
  const grantsQuery = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => apiClient.getAccountGrants(
      typeof pageParam === "string" ? pageParam : undefined,
      2,
    ),
    initialPageParam: null as string | null,
  ...
  const targetsQuery = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => apiClient.getGrantTargets(
      typeof pageParam === "string" ? pageParam : undefined,
    ),
    initialPageParam: null as string | null,
    getNextPageParam: (page) => page.next_cursor,
    ...
    <section className="space-y-6">
      ...
      {connectionsQuery.isError && (
        <div role="alert" className="space-y-2 text-sm text-destructive">
          ...
          <Button type="button" size="sm" variant="outline" onClick={() => void connectionsQuery.refetch()}>Retry</Button>
        </div>
      )}
      {!identitiesLoading && !connectionsQuery.isLoading && !connectionsQuery.isError && connectionsQuery.data?.length === 0 && (
      ...
      {isAdministrator && (
        <>
          {(accountsQuery.isError || grantsQuery.isError || targetsQuery.isError) && (
            <div role="alert" className="space-y-2 text-sm text-destructive">
          ...
          {(accountsQuery.isLoading || grantsQuery.isLoading || targetsQuery.isLoading) && (
