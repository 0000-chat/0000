...
describe("ConnectionsPage", () => {
  ...
  it("creates, narrows, and revokes a grant for the selected agent target", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Account access grants" })).toBeVisible();
    expect(await screen.findByRole("option", { name: /Agent · Agent \(agent\)/ })).toBeVisible();
    expect(await screen.findByRole("button", { name: "Load more accounts" })).toBeVisible();
    await user.click(screen.getByRole("button", { name: "Load more accounts" }));
    await user.selectOptions(screen.getByLabelText("Identity"), "membership_agent:identity_agent");
    await user.selectOptions(screen.getByLabelText("Connected account"), "account_connection_human_whatsapp");
    ...
    expect(await screen.findByText("Select chats for narrowed access")).toBeVisible();
    const chat = await screen.findByRole("checkbox", { name: /Family \(conversation_human_whatsapp_family\)/ });
    await user.click(chat);
    await user.click(screen.getByRole("button", { name: "Save narrowed access" }));
    await waitFor(() => expect(simulatedStore.accountGrants()[0]?.chat_scope).toBe("selected_chats"));

    await user.click(screen.getByRole("button", { name: "Revoke" }));
    await waitFor(() => expect(simulatedStore.accountGrants()[0]?.status).toBe("revoked"));
  });
  ...
  it("loads additional accounts while keeping the selected account and chat IDs stable", async () => {
    ...
    await user.selectOptions(screen.getByLabelText("Identity"), "membership_agent:identity_agent");
    await user.selectOptions(screen.getByLabelText("Chat scope"), "selected_chats");
    ...
    await waitFor(() => expect(simulatedStore.accountGrants()[0]).toMatchObject({
      account_id: "account_connection_human_whatsapp",
      identity_id: "identity_agent",
