...
describe("ConversationsShell", () => {
  it("defaults to All and renders every Human conversation in global recency order", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "All conversations" }),
    ).toBeVisible();
  ...
  it("filters the conversation list to the selected channel", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: "Select Telegram" }),
    );
    await waitFor(() =>
  ...
  it("keeps same-name contacts as separate channel-labelled rows in All", async () => {
    renderApp("/conversations?identity=identity_human");
    const alexRows = await screen.findAllByRole("link", {
      name: /Alex Rivera/,
    });
  ...
  it("reports an empty channel set with an accessible status", async () => {
    ...
    renderApp("/conversations?identity=identity_human");
    expect(
      await screen.findByText("No channels are available for this identity."),
    ).toBeVisible();
  });
  ...
  it("reports an empty selected channel without showing unrelated conversations", async () => {
    server.use(
      http.get(
        "*/api/v1/identities/identity_human/conversations",
        ({ request }) => {
          if (
            new URL(request.url).searchParams.get("channel_id") !==
  ...
  it("retries a conversation request after a simulated server error", async () => {
    ...
    server.use(
      http.get(
        "*/api/v1/identities/identity_human/conversations",
        ({ request }) => {
          if (new URL(request.url).searchParams.has("channel_id"))
            return passthroughResponse();
          attempts += 1;
          return attempts === 1
            ? HttpResponse.json(
            ...
            : HttpResponse.json({
                items: pilotScenario.conversations.filter(
            ...
            ? HttpResponse.json(
                {
                  error: {
                    ...
                    message: "private conversation detail",
                  },
                },
                { status: 503 },
              )
  ...
  it("resets the sequence gate when the principal changes without changing identity", async () => {
    ...
    const familyRow = () =>
      screen
        .getAllByTestId("conversation-row")
        .find(
          (row) =>
            row.getAttribute("data-conversation-id") ===
            "conversation_human_whatsapp_family",
        )!;
    const publishMessage = (lastMessagePreview: string) =>
      simulatedClient.publishMessage({
        tenantId: "tenant_pilot",
    ...
    queryClient.setQueryData(queryKeys.channels("identity_agent"), [
      "agent cache",
    ]);
    ...
    await waitFor(() =>
      expect(familyRow()).toHaveTextContent("Higher sequence update"),
    );
    ...
    await waitFor(() =>
      expect(familyRow()).toHaveTextContent("Lower sequence after reconnect"),
    );
    expect(
      queryClient.getQueryData(queryKeys.channels("identity_agent")),
    ).toEqual(["agent cache"]);
  });
  ...
  it("invalidates only the active identity for a projection change", async () => {
    ...
    const listener = subscribe.mock.calls.at(-1)?.[0] as (
      event: unknown,
    ) => void;
    ...
    expect(
      invalidate.mock.calls.every(
        ([filters]) => !filters?.queryKey?.includes("identity_agent"),
      ),
    ).toBe(true);
    expect(
      queryClient.getQueryData(queryKeys.channels("identity_agent")),
