...
import { compareConversationRecency, paginateConversations, paginateMessages } from "./conversation-pagination";
...
export class SimulatedStore {
  ...
  connectedAccounts(identityId?: string): ConnectedAccount[] {
    return this.identities()
      .flatMap((identity) => identityId !== undefined && identity.id !== identityId
        ? []
      ...
      .toSorted((left, right) => left.account_id.localeCompare(right.account_id));
  }
  ...
  accountGrants(): AccountGrant[] {
    return clone(this.grants).toSorted((left, right) => left.id.localeCompare(right.id));
  }
  ...
  createAccountGrant(input: AccountGrantMutation): AccountGrant | null {
    const account = this.connectedAccounts().find((candidate) => candidate.account_id === input.account_id);
    const target = this.grantTargets().find((candidate) =>
      candidate.membership_id === input.membership_id && candidate.identity_id === input.identity_id,
    );
    if (!account || !target) return null;
    const existing = this.grants.find((grant) =>
  ...
  updateAccountGrant(grantId: string, input: Pick<AccountGrant, "operation_scope" | "chat_scope" | "chat_ids">): AccountGrant | null {
  ...
  accountConversations(
    ...
    const account = this.connectedAccounts().find((candidate) => candidate.account_id === accountId);
    if (!account || !this.state.identities.some((candidate) => candidate.id === identityId)) return null;
    ...
    // subject.
    const conversations = this.conversations(account.identity_id, account.connection_id);
