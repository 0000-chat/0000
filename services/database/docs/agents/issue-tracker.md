...
the `service:database` label. Use the `gh` CLI with `--repo 0000-chat/0000`
for all operations.

## Conventions

- **Create an issue:** `gh issue create --repo 0000-chat/0000 --title "..." --body "..." --label service:database`.
- **Read an issue:** `gh issue view <number> --repo 0000-chat/0000 --comments`, filtering comments with `jq` and fetching labels.
- **List issues:** `gh issue list --repo 0000-chat/0000 --label service:database --state open --json number,title,body,labels,comments --jq '[.[] | {number, title, body, labels: [.labels[].name], comments: [.comments[].body]}]'`.
- **Comment on an issue:** `gh issue comment <number> --body "..."`
- **Apply or remove labels:** `gh issue edit <number> --add-label "..."` or `--remove-label "..."`
- **Close an issue:** `gh issue close <number> --comment "..."`

Do not infer the repository from `git remote -v`: the service is now nested in
the monorepo. The original issue mapping is in the migration record.
