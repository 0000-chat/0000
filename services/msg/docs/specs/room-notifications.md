...
- Suppress a push for the subscription whose private browser identity sourced the post before touching that subscription's pending work. A different new message replaces that subscription's pending or retrying row for the room; the replacement points to the new message and starts its own 24-hour deadline. Already-sending work may finish, and replacement never resets an existing event's deadline.
