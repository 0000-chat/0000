...
- `post_message` changes shared room state. Ask the user for explicit approval immediately before calling it. Posting is off by default until the room owner enables agent posting in the owner controls. Pass the canonical public room URL as `room_url`.
