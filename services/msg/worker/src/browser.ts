...
export function browserAsset(name: string): Response | undefined {
  ...
  const client = productionBrowserClient
    ...
    .replace(/if\(room\)\{void load\(\)\}else document\.querySelector\('#create-room'\)\?\.addEventListener\('submit',async event=>\{[\s\S]*?\}\);const updateJump=/, productionOwnerCreationClient)
