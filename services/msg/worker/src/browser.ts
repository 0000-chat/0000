...
import { browserFailureState, copyText, createLiveController, createOwnerControlsController, createPushEnrollmentController, createThemeController, createWebhookPanelController, handleAgentPromptCopy, normalizeOwnerManagementUrl, readPushBrowserId, type PushEnrollmentState, type WebhookPanelEntry } from "./browser-controller";
...
const productionOwnerCreationClient = String.raw`if(room){void load()}else{const createForm=document.querySelector('#create-room'),field=document.querySelector('#initial-message'),receipt=document.querySelector('#creation-receipt'),receiptHeading=document.querySelector('#creation-receipt-title'),publicLink=document.querySelector('#receipt-public-link'),ownerLink=document.querySelector('#receipt-owner-link'),continueLink=document.querySelector('#receipt-continue'),ownerStatus=document.querySelector('#owner-post-status'),enable=document.querySelector('#owner-post-enable'),rotate=document.querySelector('#owner-post-rotate'),disable=document.querySelector('#owner-post-disable'),copyOwner=document.querySelector('#owner-post-copy-owner'),openOwner=document.querySelector('#owner-post-open'),copy=document.querySelector('#owner-post-copy'),createSubmit=createForm?.querySelector('button[type="submit"]');let owner,creating=false,creationIdempotencyKey='',creationContent='';const updateOwnerButtons=state=>{if(ownerStatus)ownerStatus.textContent=state.message;if(enable)enable.disabled=state.busy||state.enabled;if(rotate)rotate.disabled=state.busy||!state.enabled;if(disable)disable.disabled=state.busy||!state.enabled;if(copyOwner)copyOwner.disabled=state.busy||!owner;if(openOwner)openOwner.disabled=state.busy||!owner;if(copy)copy.disabled=state.busy||!state.invitationAvailable};const setCreating=next=>{creating=next;if(createSubmit)createSubmit.disabled=next;if(field)field.disabled=next;if(createForm?.setAttribute)createForm.setAttribute('aria-busy',String(next))};const normalizePublicUrl=(value,roomId)=>{if(typeof value!=='string'||typeof roomId!=='string'||!roomId)return null;let parsed;try{parsed=new URL(value,location.origin)}catch{return null}if(parsed.origin!==location.origin||parsed.username||parsed.password||parsed.search||parsed.hash||parsed.pathname!=='/'+encodeURIComponent(roomId))return null;return parsed.toString()};const openManagement=url=>{const open=globalThis.open;if(typeof open==='function'){open(url,'_blank','noopener,noreferrer');return}if(typeof location.assign==='function')location.assign(url)};createForm?.addEventListener('submit',async event=>{event.preventDefault();if(creating)return;if(!field?.value.trim()){field?.focus();return}const content=field.value;if(creationContent!==content){creationContent=content;creationIdempotencyKey=crypto.randomUUID()}setCreating(true);setNotice('Creating conversation…');try{const response=await fetch('/',{method:'POST',headers:{accept:'application/json','content-type':'application/json','idempotency-key':creationIdempotencyKey},body:JSON.stringify({content,author:'Anonymous',display_name:'Anonymous',semantic_type:'message'})});if(!response.ok)throw Error();const data=await response.json();if(!data||typeof data.room?.id!=='string')throw Error();const conversationUrl=normalizePublicUrl(data.conversation_url,data.room.id);if(!conversationUrl)throw Error();const managementUrl=normalizeOwnerManagementUrl(data.manage_url,conversationUrl);const nextOwner=createOwnerControlsController({fetch:(input,init)=>fetch(input,init),manageUrl:managementUrl,onState:updateOwnerButtons,openApiUrl:new URL('/openapi.json',location.origin).toString(),publicRoomId:data.room.id,publicRoomUrl:conversationUrl});publicLink.textContent=conversationUrl;publicLink.href=conversationUrl;ownerLink.textContent=managementUrl;continueLink.href=conversationUrl;owner=nextOwner;creationIdempotencyKey='';creationContent='';if(copyOwner)copyOwner.disabled=false;if(openOwner)openOwner.disabled=false;if(receipt)receipt.hidden=false;if(createForm)createForm.hidden=true;receiptHeading?.focus();setNotice('Conversation created. Save the private owner link before continuing.')}catch{setCreating(false);setNotice('The conversation could not be created. Try again.','error')}});enable?.addEventListener('click',()=>{if(owner)void owner.enable()});rotate?.addEventListener('click',()=>{if(owner)void owner.rotate()});disable?.addEventListener('click',()=>{if(owner)void owner.disable()});copyOwner?.addEventListener('click',()=>{if(!owner)return;void owner.copyManagement(value=>copyText(value,{clipboard:globalThis.navigator?.clipboard,documentObject:document})).then(()=>say('Private owner link copied')).catch(()=>say('Copy is not available in this browser'))});openOwner?.addEventListener('click',()=>{if(owner)owner.openManagement(openManagement)});copy?.addEventListener('click',()=>{if(!owner)return;void owner.copyInvitation(value=>copyText(value,{clipboard:globalThis.navigator?.clipboard,documentObject:document})).then(()=>say('Delegated invitation copied')).catch(()=>say('Copy is not available in this browser'))})}const updateJump=`;
...
const independentOwnerCreationClient = productionOwnerCreationClient
  .replace(
    String.raw`disable=document.querySelector('#owner-post-disable'),copyOwner=`,
    String.raw`disable=document.querySelector('#owner-post-disable'),mcpEnable=document.querySelector('#owner-mcp-enable'),mcpDisable=document.querySelector('#owner-mcp-disable'),copyOwner=`,
  )
  .replace(
    String.raw`if(disable)disable.disabled=state.busy||!state.enabled;if(copyOwner)`,
    String.raw`if(disable)disable.disabled=state.busy||!state.enabled;if(mcpEnable)mcpEnable.disabled=state.busy||state.mcpEnabled;if(mcpDisable)mcpDisable.disabled=state.busy||!state.mcpEnabled;if(copyOwner)`,
  )
  .replace(
    String.raw`disable?.addEventListener('click',()=>{if(owner)void owner.disable()});`,
    String.raw`disable?.addEventListener('click',()=>{if(owner)void owner.disable()});mcpEnable?.addEventListener('click',()=>{if(owner)void owner.enableMcp()});mcpDisable?.addEventListener('click',()=>{if(owner)void owner.disableMcp()});`,
  );
...
export function browserAsset(name: string): Response | undefined {
  ...
  const client = productionBrowserClient
    ...
    .replace(/if\(room\)\{void load\(\)\}else document\.querySelector\('#create-room'\)\?\.addEventListener\('submit',async event=>\{[\s\S]*?\}\);const updateJump=/, independentOwnerCreationClient)
    .replace(/if\(room\)\{void load\(\)\}else document\.querySelector\('#create-room'\)\?\.addEventListener\('submit',async event=>\{[\s\S]*?\}\);const updateJump=/, () => String.raw`if(room){void load()}else{const createForm=document.querySelector('#create-room'),field=document.querySelector('#initial-message'),receipt=document.querySelector('#creation-receipt'),publicLink=document.querySelector('#receipt-public-link'),ownerLink=document.querySelector('#receipt-owner-link'),continueLink=document.querySelector('#receipt-continue'),ownerStatus=document.querySelector('#owner-post-status'),enable=document.querySelector('#owner-post-enable'),rotate=document.querySelector('#owner-post-rotate'),disable=document.querySelector('#owner-post-disable'),copy=document.querySelector('#owner-post-copy');let owner;const updateOwnerButtons=state=>{if(ownerStatus)ownerStatus.textContent=state.message;if(enable)enable.disabled=state.busy||state.enabled;if(rotate)rotate.disabled=state.busy||!state.enabled;if(disable)disable.disabled=state.busy||!state.enabled;if(copy)copy.disabled=state.busy||!state.invitationAvailable};createForm?.addEventListener('submit',async event=>{event.preventDefault();if(!field?.value.trim()){field?.focus();return}setNotice('Creating conversation…');try{const response=await fetch('/',{method:'POST',headers:{accept:'application/json','content-type':'application/json'},body:JSON.stringify({content:field.value,author:'Anonymous',display_name:'Anonymous',semantic_type:'message'})});if(!response.ok)throw Error();const data=await response.json();if(typeof data.conversation_url!=='string'||typeof data.manage_url!=='string'||typeof data.room?.id!=='string')throw Error();publicLink.textContent=data.conversation_url;publicLink.href=data.conversation_url;ownerLink.textContent=data.manage_url;ownerLink.href=data.manage_url;continueLink.href=data.conversation_url;owner=createOwnerControlsController({fetch:(input,init)=>fetch(input,init),manageUrl:data.manage_url,onState:updateOwnerButtons,openApiUrl:new URL('/openapi.json',location.origin).toString(),publicRoomId:data.room.id,publicRoomUrl:data.conversation_url});if(receipt)receipt.hidden=false;if(createForm)createForm.hidden=true;setNotice('Conversation created. Save the private owner link before continuing.')}catch{setNotice('The conversation could not be created. Try again.','error')}});enable?.addEventListener('click',()=>{if(owner)void owner.enable()});rotate?.addEventListener('click',()=>{if(owner)void owner.rotate()});disable?.addEventListener('click',()=>{if(owner)void owner.disable()});copy?.addEventListener('click',()=>{if(!owner)return;void owner.copyInvitation(value=>copyText(value,{clipboard:globalThis.navigator?.clipboard,documentObject:document})).then(()=>say('Delegated invitation copied')).catch(()=>say('Copy is not available in this browser'))})}const updateJump=`)
...
export function renderBrowserDocument(options: BrowserPageOptions): BrowserPageDocument {
  ...
  if (!options.room) {
    html = html.replace('<p class="state-notice" id="state-notice" aria-live="polite" hidden></p>', `${ownerCreationReceipt()}<p class="state-notice" id="state-notice" aria-live="polite" hidden></p>`);
    html = html.replace(
      '<button class="button compact" id="owner-post-disable" type="button" disabled>Disable agent posting</button>',
      '<button class="button compact" id="owner-mcp-enable" type="button" disabled>Enable anonymous MCP posting</button><button class="button compact" id="owner-mcp-disable" type="button">Disable anonymous MCP posting</button><button class="button compact" id="owner-post-disable" type="button" disabled>Disable delegated invitation</button>',
    );
  }
  if (options.room) {
    html = html.replace(
...
function ownerCreationReceipt(): string {
  return '<section class="owner-receipt agent-home-guide" id="creation-receipt" aria-labelledby="creation-receipt-title" hidden><h2 id="creation-receipt-title" tabindex="-1">Conversation created</h2><p>Save the private owner link now. It cannot be recovered after you leave or refresh this page.</p><dl class="room-facts"><div><dt>Public thread</dt><dd><a id="receipt-public-link" target="_blank" rel="noreferrer"></a></dd></div><div><dt>Private owner link</dt><dd><span id="receipt-owner-link"></span></dd></div></dl><p class="owner-post-warning">Anonymous MCP agents can post by default. Anyone holding a delegated posting capability can also write to this conversation; keep that capability secret.</p><p class="state-notice" id="owner-post-status" role="status" aria-live="polite">Anonymous MCP posting is enabled by default. Delegated posting invitation is disabled.</p><div class="composer-actions"><button class="button primary compact" id="owner-post-enable" type="button">Enable delegated invitation</button><button class="button compact" id="owner-post-rotate" type="button" disabled>Rotate delegated invitation</button><button class="button compact" id="owner-post-disable" type="button" disabled>Disable agent posting</button><button class="button compact" id="owner-post-copy-owner" type="button" disabled>Copy private owner link</button><button class="button compact" id="owner-post-open" type="button" disabled>Open private owner controls</button><button class="button compact" id="owner-post-copy" type="button" disabled>Copy delegated invitation</button><a class="button compact" id="receipt-continue" target="_blank" rel="noreferrer">Continue to public thread</a></div></section>';
}
