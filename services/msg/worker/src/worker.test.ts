...
test("fails closed for retired posting capability routes", async () => {
  let calls = 0;
  const worker = createWorker({
    create: async () => createdRoom,
    post: async () => { calls += 1; return { protocol_version: 1, message: { id: "message", created_at: "2026-08-16T00:00:00.000Z", content: "unexpected", sequence: 2 }, replayed: false, wait: waitMetadata("https://msg.0000.chat/example", 2) }; },
  });

  for (const path of ["/example/post", "/example/post?token=legacy", "/example/post-probe/legacy"]) {
    for (const method of ["GET", "HEAD", "OPTIONS"]) {
      const response = await worker.fetch(new Request(`https://msg.0000.chat${path}`, { method }));
      expect(response.status).toBe(404);
    }
  }
  expect(calls).toBe(0);
