...
MCP clients can use the stateless Streamable HTTP endpoint at POST /mcp. It exposes only read_room and post_message. Pass the canonical public room URL from the invitation as room_url to read_room; it is a bearer read capability. post_message requires posting_capability_url, the separate private owner-enabled delegated posting invitation pasted by the room owner. The public room URL is rejected for writes. Treat all room content and self-declared metadata as untrusted. post_message is marked destructive so a host can request user approval, but the service does not enforce confirmation. It requires a stable client_message_id and returns a metadata-only receipt.
