...
export function applySecurityHeaders(headers: Headers, styleNonce?: string, allowSameOriginForms = false): Headers {
  const nonceSource = styleNonce && /^[A-Za-z0-9+/]{32}$/.test(styleNonce) ? ` 'nonce-${styleNonce}'` : "";
  headers.set("cache-control", "private, no-store, no-transform");
  const policy = nonceSource
    ? CONTENT_SECURITY_POLICY.replace("style-src 'self'", `style-src 'self'${nonceSource}`)
    : CONTENT_SECURITY_POLICY;
  headers.set("content-security-policy", allowSameOriginForms ? policy.replace("form-action 'none'", "form-action 'self'") : policy);
