...
test("upgrades schema 14 rooms after restoring removed delegated-posting columns", () => {
  ...
  const roomStorage = storage(database);
  migrateRoomSchema(roomStorage);
  database.query("INSERT INTO room_state (singleton, schema_version, protocol_version, created_at, last_message_at, inactivity_expires_at, absolute_expires_at, next_sequence, message_count, total_bytes, status, tombstone_expires_at, management_hash, notification_id, get_post_hash, get_post_enabled) VALUES (1, 15, 1, 1000, 1000, 7000, 7000, 2, 1, 5, 'active', NULL, 'management-hash', 'notification-1', 'delegated-hash', 1)").run();
  ...
  // This is the schema 14 transformation: both delegated-posting columns were
  // removed, while the room and message tables otherwise remain intact.
  ...
  expect(database.query("SELECT version FROM room_schema").get()).toEqual({ version: CURRENT_ROOM_SCHEMA_VERSION });
  expect(database.query("SELECT schema_version, status, management_hash, notification_id, get_post_hash, get_post_enabled FROM room_state").get()).toEqual({
