...
import { createMsgAuthenticator, MSG_READ, MSG_WRITE, type MsgRoomAuthPort } from "./auth";
...
function responseForPlatform(): { fetch: typeof fetch; credentials: Map<string, { guestId: string; capabilities: string[]; grantId: string }>; guests: GuestRecord[] } {
  ...
  return { credentials, fetch: fetcher, guests };
}
