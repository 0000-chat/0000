...
export class ConversationRoom extends DurableObject<ConversationRoomEnv> {
  ...
  private async post(request: Request): Promise<Response> {
    ...
    const now = this.now();
    const result = this.commitMessage(input.input, key, now, undefined, input.browser_id ?? null);
    if (result.expired) {
      await this.expire(now, "Conversation expired");
  ...
  private commitMessage(
    ...
    sourceBrowserId: string | null = null,
  ) {
    return this.ctx.storage.transactionSync(() => {
      ...
      const message: StoredMessage = { ...input, ...(key ? { idempotency_key: key } : {}), byte_count: bytes, created_at: now, id, sequence: state.next_sequence, source_browser_id: sourceBrowserId };
