...
export const createGrantRoute = createRoute({
  ...
  request: {
    body: {
      content: { "application/json": { schema: AccountGrantMutationSchema } },
    },
  },
  responses: grantResponses(
    AccountGrantSchema,
    "Created account and chat grant",
  ),
});
...
export const updateGrantRoute = createRoute({
  ...
  request: {
    ...
    body: {
      content: { "application/json": { schema: AccountGrantUpdateSchema } },
    },
  },
  responses: grantResponses(
    AccountGrantSchema,
    "Updated account and chat grant",
  ),
});
...
export const revokeGrantRoute = createRoute({
  ...
  responses: grantResponses(
    AccountGrantSchema,
    "Revoked account and chat grant",
  ),
});
...
export const grantTargetsRoute = createRoute({
  ...
  responses: grantResponses(
    AccountGrantTargetPageSchema,
    "Eligible account grant targets",
  ),
});
...
export const createPermissionRequestRoute = createRoute({
  ...
  request: {
    body: {
      content: {
        "application/json": { schema: PermissionRequestCreateSchema },
      },
    },
  },
  responses: grantResponses(
...
const grantFailure = (context: Context<GrantRouteEnv>, error: unknown) => {
  if (error instanceof GrantRepositoryError) {
    if (error.code === "grant_invalid")
      return publicError(context, 400, "invalid_request", "Invalid grant data");
    if (error.code === "grant_not_found")
      return publicError(context, 404, "not_found", "Grant not found");
    if (error.code === "grant_conflict")
      return publicError(
        context,
        409,
        "invalid_request",
        "Grant mutation conflict",
      );
  }
  return publicError(
    context,
...
const ownIdentity = (
  ...
  context
    .get("authorization")
    .identities.some((identity) => identity.identity_id === identityId);

export const grantsHandler: Handler<
  GrantRouteEnv,
  string,
  {
    out: {
...
> = async (context) => {
  try {
    ...
    if (
      !admin &&
      query.identity_id !== undefined &&
      !ownIdentity(context, query.identity_id)
    ) {
      return publicError(
        context,
        403,
        "forbidden",
        "An agent may inspect only its own grants",
      );
    }
    const visibleIdentityId = admin
      ? query.identity_id
      : (query.identity_id ??
        context.get("authorization").identities[0]?.identity_id);
    if (!admin && visibleIdentityId === undefined) {
      return context.json({ items: [], next_cursor: null }, 200);
    }
    return context.json(
      await listAccountGrants(
        context.env.CONTROL_DB.withSession("first-primary"),
        {
          tenantId: context.get("authorization").tenant.id,
          ...(admin
            ? {}
            : { membershipId: context.get("authorization").membership.id }),
          ...(visibleIdentityId === undefined
            ? {}
            : { identityId: visibleIdentityId }),
...
export const createGrantHandler: Handler<
  GrantRouteEnv,
  string,
  { out: { json: AccountGrantMutation } }
> = async (context) => {
  ...
  try {
    ...
    return context.json(
      await createAccountGrant(context.env.CONTROL_DB, {
...
export const updateGrantHandler: Handler<
  GrantRouteEnv,
  string,
  { out: { param: { grant_id: string }; json: AccountGrantUpdate } }
> = async (context) => {
  const denied = requireAdministrator(context);
  if (denied) return denied;
  try {
    const body = context.req.valid("json");
    return context.json(
      await updateAccountGrant(context.env.CONTROL_DB, {
        idempotencyKey: body.idempotency_key,
        tenantId: context.get("authorization").tenant.id,
        actorPrincipalId: context.get("authorization").principal.id,
...
export const revokeGrantHandler: Handler<
  GrantRouteEnv,
  string,
  { out: { param: { grant_id: string } } }
> = async (context) => {
  const denied = requireAdministrator(context);
  if (denied) return denied;
  try {
    ...
    return context.json(
      await revokeAccountGrant(context.env.CONTROL_DB, {
        idempotencyKey: key,
        tenantId: context.get("authorization").tenant.id,
        actorPrincipalId: context.get("authorization").principal.id,
        grantId: context.req.valid("param").grant_id,
...
export const accountsHandler: Handler<
  GrantRouteEnv,
  string,
  { out: { query: { identity_id?: string; cursor?: string; limit?: number } } }
> = async (context) => {
  try {
    const query = context.req.valid("query");
    const admin = isAdministrator(context);
    if (
      ...
      return publicError(
        ...
        403,
        "forbidden",
        "An agent may inspect only its own accounts",
      );
    }
    ...
    return context.json(
      await listConnectedAccounts(
        context.env.CONTROL_DB.withSession("first-primary"),
        {
          ...
          ...(admin && query.identity_id !== undefined
            ? { identityId: query.identity_id }
            : {}),
...
export const grantTargetsHandler: Handler<
  GrantRouteEnv,
  string,
...
export const permissionRequestsHandler: Handler<
  GrantRouteEnv,
  string,
  {
    out: {
  ...
  try {
    ...
    if (
      ...
      return publicError(
        ...
        403,
        "forbidden",
        "An agent may inspect only its own requests",
      );
    }
    const visibleIdentityId = admin
      ? query.identity_id
...
export const createPermissionRequestHandler: Handler<
  GrantRouteEnv,
  string,
  { out: { json: PermissionRequestCreate } }
> = async (context) => {
  try {
    const body = context.req.valid("json");
    if (!ownIdentity(context, body.identity_id)) {
      return publicError(
        ...
        403,
        "forbidden",
        "A principal may request access only for its own identity",
      );
    }
    return context.json(
      await createPermissionRequest(context.env.CONTROL_DB, {
        idempotencyKey: body.idempotency_key,
        tenantId: context.get("authorization").tenant.id,
