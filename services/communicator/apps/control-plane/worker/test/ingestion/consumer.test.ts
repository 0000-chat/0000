...
const archiveFor = async (
  events: readonly ProjectionEventEnvelope[] = [
    eventFor("$consumer-valid:example"),
  ],
  options: {
    tenantId?: string;
    batchId?: string;
  ...
  archiveCounter += 1;
  const batchId =
    options.batchId ?? `batch_${archiveCounter.toString(16).padStart(64, "0")}`;
...
describe("committed archive queue consumer", () => {
  ...
  it("reads one exact archive pair, initializes, applies once, and ACKs after success", async () => {
    ...
    const result = await runWorkerBatch([
      { id: "consumer-valid", body: pointer },
    ]);
    ...
    await expect(
      (await import("cloudflare:test")).runInDurableObject(
        projection,
  ...
  it.each([
    ...
    [
      "raw-looking checkpoint",
      { kind: "matrix_sync_token", value: "raw-token" },
    ],
  ] as const)(
    "rejects %s before any DO access",
    async (_name, sourceCheckpoint) => {
      const { committed, pointer } = await archiveFor(
        [eventFor("$consumer-invalid-checkpoint:example")],
        { sourceCheckpoint },
      );
      const getProjection =
        vi.fn<NonNullable<IngestionConsumerServices["getTenantProjection"]>>();
  ...
  it.each([
    ...
    "retries an R2 %s result with the exact delay",
    async (_name, code, delay) => {
      const fixture = await createMatrixFixture();
      const readCommittedArchiveBatch = vi
        .fn<
          NonNullable<IngestionConsumerServices["readCommittedArchiveBatch"]>
        >()
  ...
  it.each([
    ...
    "rejects a pointer %s mismatch before D1 or DO access",
    async (_name, mutate) => {
      const fixture = await createMatrixFixture();
      const readCommittedArchiveBatch = vi.fn(async () => ({
  ...
  it.each([
    ...
    "rejects a committed manifest %s mismatch before D1 or DO access",
    async (_name, patch) => {
      const fixture = await createMatrixFixture();
      const readCommittedArchiveBatch = vi.fn(async () => ({
  ...
  it.each([
    ...
    "retries a historical route %s result with the exact delay",
    async (_name, routeResult, delay) => {
      const fixture = await createMatrixFixture();
      const resolveArchivedIngestionRoute = vi
        .fn<
          NonNullable<
            IngestionConsumerServices["resolveArchivedIngestionRoute"]
          >
        >()
  ...
  it.each([
    ...
    "retries historical bindings %s with the exact delay",
    async (_name, bindingsResult, delay) => {
      const fixture = await createMatrixFixture();
        ...
        .mockImplementation(
          async () =>
            bindingsResult as Awaited<
              ReturnType<
                NonNullable<
                  IngestionConsumerServices["resolveArchivedBindings"]
                >
              >
            >,
        );
  ...
  it.each([
    ...
    "retries a DO initialize %s with the exact delay and skips apply",
    async (_name, code, delay) => {
      const fixture = await createMatrixFixture();
  ...
  it.each([
    ...
    "retries a DO apply %s with the exact delay after one initialize",
    async (_name, code, delay) => {
      const fixture = await createMatrixFixture();
