...
describe("tenant projection control", () => {
  it("materializes Stage C control and deletion event families", async () => {
    ...
    await expect(
      rows(stub, "SELECT id, status, failure_code FROM commands"),
    ).resolves.toEqual([
      {
        id: "command_a",
        status: "failed",
    ...
    await expect(
      rows(
        stub,
        "SELECT target_event_id, tombstone_event_id, tombstone_type FROM event_tombstones ORDER BY target_event_id",
      ),
    ).resolves.toEqual([
      {
        target_event_id: "target_event_a",
        tombstone_event_id: "stage_c_replay_marker",
        tombstone_type: "replay.tombstone",
      },
      {
        target_event_id: "target_event_b",
        tombstone_event_id: "stage_c_correction_marker",
    ...
    await expect(
      rows(
        stub,
        "SELECT resource_type, resource_id, reason_code FROM resource_tombstones",
      ),
    ).resolves.toEqual([
      {
  ...
  it("projects command observations with observed LWW and earliest occurrence", async () => {
    ...
    await stub.applyBatch(
      input(
        [
          commandUpdated("command_newer", "command_lww", {
            tenant_id: tenant,
    ...
    await stub.applyBatch(
      input(
        [
          commandUpdated("command_older", "command_lww", {
            tenant_id: tenant,
    ...
    await expect(
      rows(
        stub,
        "SELECT operation, delivery_mode, status, failure_code, created_at, updated_at, last_event_id FROM commands WHERE id = ?",
        "command_lww",
      ),
    ).resolves.toEqual([
      {
    ...
    await stub.applyBatch(
      input(
        [
          commandUpdated("command_cleared", "command_lww", {
            tenant_id: tenant,
    ...
    await expect(
      rows(
        stub,
        "SELECT delivery_mode, status, failure_code, created_at, updated_at, last_event_id FROM commands WHERE id = ?",
        "command_lww",
      ),
    ).resolves.toEqual([
      {
    ...
    await expect(
      rows(
        stub,
        "SELECT delivery_mode, status, failure_code, last_event_id FROM commands WHERE id = ?",
        "command_lww",
      ),
    ).resolves.toEqual([
      {
    ...
    await stub.applyBatch(
      input(
        [
          commandUpdated("command_failed_after_delete", "command_lww", {
            tenant_id: tenant,
            observed_at: "2026-09-07T05:00:01.000Z",
          }),
        ],
    ...
    await stub.applyBatch(
      input(
        [
          commandUpdated("command_failed_post_delete", "command_lww", {
            tenant_id: tenant,
    ...
    await expect(
      rows(
        stub,
        "SELECT status, failure_code FROM commands WHERE id = ?",
        "command_lww",
      ),
    ).resolves.toEqual([
      {
  ...
  it("reconciles delivery observations before and after message creation", async () => {
    ...
    await stub.applyBatch(
      input(
        [
          deliveryUpdated("delivery_before", "message_delivery_before", {
            tenant_id: tenant,
    ...
    await expect(
      rows(
        stub,
        "SELECT delivery_status, delivery_failure_code, delivery_observed_ms, delivery_event_id FROM messages WHERE id = ?",
        "message_delivery_before",
      ),
    ).resolves.toEqual([
      {
        delivery_status: "failed",
    ...
    await stub.applyBatch(
      input(
        [
          deliveryUpdated("delivery_nonfailed", "message_delivery_before", {
            tenant_id: tenant,
            occurred_at: "2026-09-07T03:00:00.000Z",
            observed_at: "2026-09-07T03:00:01.000Z",
            payload: {
    ...
    await expect(
      rows(
        stub,
        "SELECT delivery_status, delivery_failure_code FROM messages WHERE id = ?",
        "message_delivery_before",
      ),
    ).resolves.toEqual([
      {
    ...
    await expect(
      rows(
        stub,
        "SELECT delivery_status, delivery_failure_code, delivery_event_id FROM messages WHERE id = ?",
        "message_delivery_before",
      ),
    ).resolves.toEqual([
      {
    ...
    await stub.applyBatch(
      input(
        [
          deliveryUpdated("delivery_after_delete", "message_delivery_before", {
            tenant_id: tenant,
            observed_at: "2026-09-07T06:00:01.000Z",
          }),
        ],
    ...
    await expect(
      rows(
        stub,
        "SELECT delivery_status, failure_code FROM message_delivery_updates WHERE message_id = ?",
        "message_delivery_before",
      ),
    ).resolves.toEqual([
      {
    ...
    await expect(
      rows(
        stub,
        "SELECT delivery_failure_code FROM messages WHERE id = ?",
        "message_delivery_before",
      ),
    ).resolves.toEqual([{ delivery_failure_code: null }]);
  });
  ...
  it("keeps event markers audit-only and resolves repeated winners by observed tuple", async () => {
    ...
    await stub.applyBatch(
      input(
        [markerNew, markerOld].map((item) => ({ ...item, tenant_id: tenant })),
        { tenant_id: tenant },
      ),
    );
    await stub.applyBatch(
    ...
    await expect(
      rows(
        ...
        "message_marker_target",
      ),
    ).resolves.toEqual([{ body: "still visible" }]);
    await expect(
      rows(
        stub,
        "SELECT target_event_id, tombstone_event_id, tombstone_type, reason_code, occurred_at FROM event_tombstones",
      ),
    ).resolves.toEqual([
      {
        target_event_id: "marker_target",
  ...
  it("rejects Stage C owner reuse atomically across commands, delivery, markers, and deletions", async () => {
    ...
    const inputB = (events: ProjectionEventEnvelope[]) =>
      input(events, {
        tenant_id: tenant,
        authorization: auth(
          ["projection.write"],
    ...
    await stub.applyBatch(
      input(
        [
          commandUpdated("owner_command_a", "owner_command", {
            tenant_id: tenant,
          }),
        ],
        { tenant_id: tenant },
    ...
    await stub.applyBatch(
      input(
        [
          deliveryUpdated("owner_delivery_a", "owner_delivery_message", {
            tenant_id: tenant,
          }),
        ],
        { tenant_id: tenant },
    ...
    await stub.applyBatch(
      input([created("owner_message_create", { tenant_id: tenant })], {
        tenant_id: tenant,
      }),
    );
    await expectCode(
    ...
    ).resolves.toEqual([{ failure_code: "temporary" }]);
  });
  ...
  it("rejects pending delivery IDs reused by commands", async () => {
    ...
    await stub.applyBatch(
      input(
        [
          deliveryUpdated("pending_delivery_id", "shared_resource_id", {
            tenant_id: tenant,
          }),
        ],
        { tenant_id: tenant },
    ...
    await expectCode(
      ...
      input(
        [
          commandUpdated("pending_delivery_command", "shared_resource_id", {
            tenant_id: tenant,
          }),
        ],
        { tenant_id: tenant },
    ...
    await expect(
      rows(stub, "SELECT COUNT(*) AS count FROM commands"),
    ).resolves.toEqual([{ count: 0 }]);
    await expect(
      rows(stub, "SELECT message_id FROM message_delivery_updates"),
    ).resolves.toEqual([{ message_id: "shared_resource_id" }]);
  });
  ...
  it("rejects marker ownership mismatch both before and after its target", async () => {
    ...
    await beforeStub.applyBatch(
      input(
        [
          eventMarker(
            "marker_before_owner",
            "replay.tombstone",
            "marker_before_target",
            {
    ...
    await expect(
      rows(beforeStub, "SELECT COUNT(*) AS count FROM applied_events"),
    ).resolves.toEqual([{ count: 1 }]);

    const afterTenant = "tenant_projector_stage_c_marker_after_owner";
    const afterStub = await initialize(afterTenant);
    await afterStub.applyBatch(
      input(
        [
          created("marker_after_target", {
    ...
    await expect(
      rows(afterStub, "SELECT COUNT(*) AS count FROM event_tombstones"),
    ).resolves.toEqual([{ count: 0 }]);
  });
});
