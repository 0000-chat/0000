...
test("switches identities symmetrically in Connections and Conversations", async ({
  page,
}) => {
  ...
  await expect(page.getByText("Agent WhatsApp", { exact: true })).toHaveCount(
    0,
  );
  ...
  await expect(page.getByRole("link", { name: /Agent Test Chat/ })).toHaveCount(
    0,
  );
  await page.getByLabel("Active identity").selectOption("identity_agent");
  await expect(
