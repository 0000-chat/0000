...
test("retries an accepted direct command with the same key after response loss", async ({
  page,
}) => {
  await page.goto(
      ...
      if (
        ...
        if (firstResponse) {
          ...
          if (response.status !== 202) {
            throw new Error(
              `Expected simulated acceptance, received ${response.status}`,
            );
          }
          return new Response(
            JSON.stringify({
  ...
  await page
    ...
    .fill("Hello from the simulated Human identity");
  await page
    .getByRole("combobox", { name: "Delivery mode" })
    .selectOption("direct");
  const sendButton = page.getByRole("button", { name: "Send message" });
  await sendButton.click();
  await expect(
    page.getByRole("status").filter({ hasText: "could not be accepted" }),
  ).toBeVisible();
  await sendButton.click();
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("list", { name: "Command activity" }).getByRole("listitem"),
  ).toHaveCount(2);
...
test("previews paced delivery and records its accepted command phase", async ({
  page,
}) => {
  await page.goto(
    "/conversations/conversation_human_telegram_alex?identity=identity_human&channel=connection_human_telegram",
...
test("shows the attention-required scenario and can reset it from System", async ({
  page,
}) => {
  await page.goto("/connections");
  await expect(
    page.getByRole("status").filter({ hasText: "SIMULATED DATA" }),
  ).toBeVisible();
  ...
  const identitySwitcher = page.getByRole("combobox", {
  ...
  await expect(
    page
      ...
      .getByRole("button", { name: /Simulation only.*Reconnect/ }),
  ).toBeDisabled();

  await page.goto("/system");
  await page.getByRole("button", { name: "Reset simulated scenario" }).click();
  await expect(
    page.getByRole("status", { name: "Simulation reset complete" }),
  ).toBeVisible();
});
