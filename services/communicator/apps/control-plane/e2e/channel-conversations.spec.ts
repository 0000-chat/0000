...
test("All and channel views preserve recency, labels, and separate contacts", async ({
  page,
}) => {
...
test("identity and channel URL guesses fail symmetrically", async ({
  page,
}) => {
  await page.goto(
    "/conversations/conversation_agent_one?identity=identity_human&channel=connection_human_whatsapp",
  );
  ...
  await page.goto(
    "/conversations/conversation_human_whatsapp_family?identity=identity_agent&channel=connection_agent_whatsapp",
  );
  await expect(page.getByRole("alert")).toContainText(
    "This conversation is unavailable.",
  );
...
test("message activity updates All without moving channel navigation", async ({
  page,
}) => {
  await page.goto("/conversations?identity=identity_human");
  const channelNavigation = page
    .getByRole("navigation", { name: "Conversation channels" })
    .filter({ visible: true });
  await expect(
    channelNavigation.getByRole("button", { name: /All.*10/ }),
  ).toBeVisible();
  await expect(
    channelNavigation
      .locator('[data-channel-id="connection_human_whatsapp"]')
      .getByRole("button")
      .first(),
  ).toContainText("Personal WhatsApp");
  await expect(
    channelNavigation
      .locator('[data-channel-id="connection_human_whatsapp"]')
      .getByRole("button")
  ...
  await expect(
    channelNavigation
      .locator('[data-channel-id="connection_human_telegram"]')
      .getByRole("button")
      .first(),
  ...
  await expect(
    channelNavigation
      .locator('[data-channel-id="connection_human_messenger"]')
      .getByRole("button")
      .first(),
  ).toContainText("Messenger");
  await expect(
    channelNavigation
      .locator('[data-channel-id="connection_human_messenger"]')
      .getByRole("button")
  ...
  await expect(
    channelNavigation.getByRole("button", { name: /All.*11/ }),
  ).toBeVisible();
  await expect(
    channelNavigation
      .locator('[data-channel-id="connection_human_messenger"]')
  ...
  const labels = await channelNavigation
  ...
  await expect(page.locator("body")).not.toContainText(
    /rt1_|wss?:\/\/|bearer|Access/,
  );
});

test("manual channel order survives ordinary navigation", async ({ page }) => {
  await page.goto("/conversations?identity=identity_human");
  const reorderTelegram = page.getByRole("button", {
...
test("pointer channel drag persists after ordinary navigation", async ({
  page,
}) => {
  await page.goto("/conversations?identity=identity_human");
  const channelNavigation = page
    .getByRole("navigation", { name: "Conversation channels" })
    .filter({ visible: true });
  const source = channelNavigation.getByTestId(
    "channel-drag-handle-connection_human_telegram",
  );
  const target = channelNavigation
    .locator('[data-channel-id="connection_human_whatsapp"]')
    .first();
  ...
  await expect(
    channelNavigation.locator("[data-channel-id]").first(),
  ).toHaveAttribute("data-channel-id", "connection_human_telegram");
  await page
    .getByRole("navigation", { name: "Primary navigation" })
    .getByRole("link", { name: "Activity", exact: true })
    .click();
...
test("identity switch replaces every scoped surface", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Alex Rivera", level: 1 }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("button", { name: "Select Agent WhatsApp" }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: /Agent Test Chat/ }),
  ).toBeVisible();
  await expect(
    page.getByText("Personal WhatsApp", { exact: true }),
  ).toHaveCount(0);
...
test("direct and paced sends stay on the opened connection", async ({
  page,
}) => {
  await page.goto(
    "/conversations/conversation_human_telegram_alex?identity=identity_human&channel=connection_human_telegram",
  ...
  const directResponse = page.waitForResponse(
    (response) =>
      response
        .url()
        .includes(
          "/api/v1/conversations/conversation_human_telegram_alex/messages",
        ) &&
  ...
  await expect(
    page
      .getByRole("status")
      .filter({ hasText: "Accepted — awaiting messaging confirmation" }),
  ).toBeVisible();

  await message.fill("A simulated paced message");
  await mode.selectOption("paced");
  const pacedResponse = page.waitForResponse(
    (response) =>
      response
        .url()
        .includes(
  ...
  const commands = await page.evaluate(async () => {
    const [commandsResponse, conversationResponse] = await Promise.all([
      ...
      fetch(
        "/api/v1/identities/identity_human/conversations/conversation_human_telegram_alex",
      ),
    ]);
    const commands = (await commandsResponse.json()) as Array<{
    ...
    return {
      sent: commands.filter(
        (command) =>
          command.conversation_id === "conversation_human_telegram_alex",
      ),
      connectionId: conversation.connection_id,
    };
  });
  expect(commands.connectionId).toBe("connection_human_telegram");
  expect(commands.sent.map((command) => command.delivery_mode)).toEqual([
    "direct",
    "paced",
  ]);
});
...
test("attention history remains readable but unsendable", async ({ page }) => {
  ...
  await expect(
    page
      .getByRole("list", { name: "Message timeline" })
      .getByText("The render is ready"),
  ).toBeVisible();
  await expect(page.getByRole("textbox", { name: "Message" })).toBeDisabled();
  await expect(
    page.getByRole("combobox", { name: "Delivery mode" }),
  ).toBeDisabled();
  await expect(
...
test("mobile channel selection and back navigation work", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/channel=connection_human_telegram/);
  await expect(
    page.getByRole("button", { name: "Channel: Telegram" }),
  ).toBeVisible();
  ...
  await expect(page).toHaveURL(
    /\/conversations\?identity=identity_human&channel=connection_human_telegram/,
  );
...
test("tablet and desktop expose the intended panes", async ({ page }) => {
  ...
  await expect(
    page.getByRole("list", { name: "Conversation inbox" }),
  ).toBeVisible();
  await expect(
    page.getByText("Select a conversation to view its messages."),
  ).toBeVisible();

  await expect(page.getByTestId("conversation-workspace")).toHaveClass(
    /overflow-hidden/,
  );
  ...
  await expect(page.getByTestId("conversation-workspace")).not.toHaveClass(
    /rounded/,
  );
});

test("desktop messenger regions scroll independently and keep the thread composer anchored", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto(
  ...
  const scrollRegions = page.locator(
    "[data-testid='conversation-workspace'] .overflow-y-auto",
  );
  await expect(scrollRegions).toHaveCount(3);
  const overflowValues = await scrollRegions.evaluateAll((elements) =>
    elements.map((element) => getComputedStyle(element).overflowY),
  );
  expect(overflowValues).toEqual(["auto", "auto", "auto"]);
  await expect(page.getByTestId("message-viewport")).toHaveCSS(
    "overflow-y",
    "auto",
  );
  await expect(
    page
      .getByTestId("message-viewport")
...
test("simulated browser never contacts a live service", async ({ page }) => {
  ...
  page.on("request", (request) => {
    ...
    if (
      ...
      (request.resourceType() === "fetch" && !path.startsWith("/api/v1/"))
    ) {
