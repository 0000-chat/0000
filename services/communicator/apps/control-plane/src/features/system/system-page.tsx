...
export function SystemPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = realtime.subscribe(() =>
      setLastSequence(realtime.lastSequence),
    );
    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <DiagnosticCard
          ...
          value={
            realtime ? realtimeStatusLabel(realtimeStatus) : "Not configured"
          }
        />
        <DiagnosticCard
          label="Last sequence"
          value={realtime ? String(lastSequence) : "—"}
        />
        <DiagnosticCard label="Fixture reset time" value={fixtureResetTime} />
        <DiagnosticCard
          label="Active connection count"
          value={String(connectionsQuery.data?.length ?? 0)}
        />
      </div>
      ...
      {simulatedBuild && (
        <div className="rounded-xl border border-dashed p-4">
          ...
          {resetMessage && (
            <p
              role="status"
              aria-label="Simulation reset complete"
