...
- Receipts always upsert the newest scoped row, even before a message exists. A local read sets `local_read_at=event.occurred_at` and unread zero when the message exists; when a message later arrives it consults the newest local read receipt and is born read. Remote receipts never alter local unread.
