...
Communicator connects Matrix-derived messaging events to tenant-scoped
application state. Synapse remains the operational messaging record. R2 holds
the immutable replay archive, and one SQLite Durable Object per tenant holds a
rebuildable projection. The ingestion Queue carries only a pointer to a
committed R2 batch.
...
Engine and Docker Compose v2 before running the full `pnpm test:python` gate.
...
The future Matrix Gateway will decrypt E2EE on a VPS and send normalized,
one-tenant batches to the private ingestion endpoint. Session, device, access,
and other Matrix keys stay on that VPS and never enter Cloudflare.
