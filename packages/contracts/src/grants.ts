...
export const AccountGrantTargetSchema = z
  .object({
...
export const AccountGrantTargetPageSchema = z
  .object({
    items: z.array(AccountGrantTargetSchema),
    next_cursor: z.string().min(1).max(2_048).nullable(),
  })
  .strict();
export type AccountGrantTargetPage = z.infer<
  typeof AccountGrantTargetPageSchema
>;
...
export const AccountGrantChatScopeSchema = z.enum([
  "all_chats",
  "selected_chats",
]);
...
export const AccountGrantSchema = z
  .object({
...
export const AccountGrantPageSchema = z
  .object({
...
export const ConnectedAccountPageSchema = z
  .object({
...
export const AccountGrantMutationSchema = z
  .object({
    membership_id: CommunicatorIdSchema,
    identity_id: CommunicatorIdSchema,
    account_id: CommunicatorIdSchema,
...
export const AccountGrantUpdateSchema = z
  .object({
    operation_scope: AccountGrantOperationScopeSchema,
    chat_scope: AccountGrantChatScopeSchema,
    chat_ids: z.array(CommunicatorIdSchema).max(MAX_GRANT_CHAT_IDS),
...
export const PermissionRequestPageSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (value.chat_scope === "all_chats" && value.chat_ids.length > 0) {
      context.addIssue({
        code: "custom",
        path: ["chat_ids"],
        message: "All-chats requests cannot include selected chat IDs",
      });
    }
    if (value.chat_scope === "selected_chats" && value.chat_ids.length === 0) {
      context.addIssue({
        code: "custom",
        path: ["chat_ids"],
        message: "Selected-chat requests require at least one chat ID",
      });
    }
  });
export type PermissionRequestCreate = z.infer<
